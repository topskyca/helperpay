...
(function () {
  ...
  function setupFormHtml(today, adding) {
    return '<div class="card onboarding-card">' +
      ...
      '<p class="eyebrow">' + (adding ? L('New helper', '新增外傭') : L('One-minute setup', '一分鐘設定')) + '</p>' +
      '<h1>' + (adding ? L('Add another helper', '加入另一位外傭') : L('Tell us the basics', '先填幾項基本資料')) + '</h1>' +
      '<p>' + L(
        adding
          ? 'This helper will have a separate calendar, salary statements and payment history.'
          : 'Answer five everyday questions. You can start using HelperPay immediately; no old records are needed.',
        adding
          ? '這位外傭會有獨立的日曆、薪金結算單及付款記錄。'
          : '只需回答五個日常問題，便可立即開始使用；毋須補錄以前的記錄。'
      ) + '</p></div>' +
      ...
      '<label for="su-helper">' + L('Helper’s name', '外傭姓名') + '</label><input id="su-helper" autocomplete="name" required placeholder="' + L('e.g. Maria', '例如：Maria') + '">' +
      ...
      '<p class="muted small">' + L('Sunday is selected. Change it only if your helper normally rests on another day.', '已預設星期日；如外傭通常在其他日子休息，才需要更改。') + '</p>' +
  ...
  function bindSetup() {
    ...
    $('#su-create').onclick = () => {
      ...
      if (!$('#su-helper').value.trim()) { setupError(L('Enter the helper’s name.', '請填寫外傭姓名。'), '#su-helper'); return; }
  ...
  function openBasicProfileSheet() {
    ...
    const ov = openSheet('<h2>' + L('Add the basics', '補充基本資料') + '</h2><p class="muted">' +
      ...
      '<label for="basic-helper">' + L('Helper’s name', '外傭姓名') + '</label><input id="basic-helper" autocomplete="name" value="' + esc(config.helperName || '') + '">' +
    ...
    ov.querySelector('#basic-save').onclick = () => {
      ...
      if(!name){fail(L('Enter the helper’s name.','請填寫外傭姓名。'),ov.querySelector('#basic-helper'));return;}
    ...
    if (!employed) {
      ...
      dayCard = '<div class="card today-routine">' +
        ...
        '<p class="muted small mt">' + L(
          ...
          '外傭如常上班便毋須記錄。只有請假、提早下班或其他例外情況才需要填寫。'
        ) + '</p>' +
  ...
  function settingsHtml() {
    ...
    ids.forEach(id => {
      ...
      const name = (cfg && cfg.helperName) || L('My helper', '我的外傭');
