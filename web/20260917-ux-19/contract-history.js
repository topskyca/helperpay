...
(function (global) {
  ...
  const validRest = row => Number.isInteger(row.restDayWeekday) && row.restDayWeekday >= 0 && row.restDayWeekday <= 6 &&
    ['paid', 'unpaid'].includes(row.restDayPayTerm) && ['no_extra_payment', 'agreed_payment', 'substituted_rest_day'].includes(row.restDayWorkArrangement) &&
  ...
  function prepareRest(config, entered, meta) {
    ...
    if (!['change', 'correction', 'confirm'].includes(meta.kind)) add('kind', 'intent');
    const reason = String(meta.reason || '').trim() || (meta.kind === 'confirm' ? 'Initial rest-day arrangement confirmed' : '');
    // A first-time completion is not a correction and should not ask a new
    // user to invent a reason. Later changes and corrections remain auditable.
    ...
    nextRows.forEach((row, i) => {
      ...
      if (!['no_extra_payment', 'agreed_payment', 'substituted_rest_day'].includes(row.restDayWorkArrangement)) add('work-' + i, 'work');
