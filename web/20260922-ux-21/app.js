...
(function () {
  ...
  function setupFormHtml(today, adding) {
    return '<div class="card onboarding-card">' +
      ...
      '<p class="eyebrow">' + (adding ? L('New helper', '新增工人姐姐') : L('Welcome to HelperPay', '歡迎使用 HelperPay')) + '</p>' +
      '<h1>' + (adding ? L('Add another helper', '加入另一位工人姐姐') : L('Track your helper’s work and monthly pay', '記錄工人姐姐的工作安排，掌握每月薪金')) + '</h1>' +
      '<p>' + L(
        adding
          ? 'This helper will have a separate calendar, salary statements and payment history.'
          : 'Enter a few basic details to get started. After that, record only rest-day work, statutory holidays, leave or anything else that differs from the usual schedule. HelperPay will update the monthly pay estimate; you do not need to enter old records first.',
        adding
          ? '這位工人姐姐會有獨立的日曆、薪金結算單及付款記錄。'
          : '先填寫幾項基本資料，即可開始使用。日後只需記錄休息日上班、法定假日、請假或其他特殊情況，HelperPay 便會更新每月薪金估算；毋須先補錄以往記錄。'
      ) + '</p></div>' +
      '<p id="setup-section-status" class="onboarding-note" role="status">' + L('Language can be changed at any time from the top right.', '可隨時在右上角切換語言。') + '</p>' +
      '<fieldset class="setup-section" data-setup-section="1"><legend class="sr-only">' + L('Basic profile', '基本檔案') + '</legend>' +
      '<input id="su-start" type="hidden" value="' + today + '">' +
      '<label for="su-helper">' + L('Helper’s name', '工人姐姐姓名') + '</label><input id="su-helper" autocomplete="name" required placeholder="' + L('e.g. Maria', '例如：Maria') + '">' +
      '<label for="su-rest">' + L('Usual day off', '通常休息日') + '</label><select id="su-rest" required>' +
      weekdays().map((w, i) => '<option value="' + i + '"' + (i === 0 ? ' selected' : '') + '>' + w + '</option>').join('') + '</select>' +
      '<p class="muted small">' + L('Sunday is selected. Change it only if your helper normally rests on another day.', '已預設星期日；如工人姐姐通常在其他日子休息，才需要更改。') + '</p>' +
      '<label for="su-wage">' + L('Current monthly wage (HK$)', '目前月薪（港幣）') + '</label><input id="su-wage" type="number" inputmode="decimal" min="0.01" step="0.01" required placeholder="5100">' +
      '<label for="su-food-mode">' + L('Food arrangement', '膳食安排') + '</label><select id="su-food-mode" required>' +
      '<option value="provided" selected>' + L('I provide food free of charge', '我免費提供膳食') + '</option><option value="allowance">' + L('I pay a monthly food allowance', '我每月支付膳食津貼') + '</option></select>' +
      '<label for="su-food">' + L('Monthly food allowance (HK$)', '每月膳食津貼（港幣）') + '</label><input id="su-food" type="number" inputmode="decimal" min="0" step="0.01">' +
      '<label for="su-contract-start">' + L('Contract first work day (optional)', '合約首個工作日（選填）') + '</label><input id="su-contract-start" type="date" max="' + today + '">' +
      '<p class="muted small">' + L('Leave this blank if the contract is not at hand. You can record this month now. To record an earlier month, add the actual first work day later in Settings.', '合約不在手邊可先留空；現在仍可補記本月。如要記錄更早月份，須稍後在「設定」填寫實際首個工作日。') + '</p></fieldset>' +
      '<p id="setup-error" class="form-error" role="alert" hidden></p>' +
      (adding ? '' : '<p class="onboarding-note mt">' + L(
      ...
      ) + '<a href="' + pageLink('privacy.html') + '" target="_blank" rel="noopener">' + L('Privacy details', '私隱詳情') + '</a></p>') +
      '<button class="btn mt" id="su-create">' + L('Start using HelperPay', '開始使用 HelperPay') + '</button>' +
  ...
  function bindSetup() {
    ...
    $$('#view input[id^="su-"], #view select[id^="su-"]').forEach(field => {
      ...
      field.required = ['su-helper', 'su-rest', 'su-wage', 'su-food-mode'].includes(field.id);
    });
    ...
    const employmentStart = () => $('#su-contract-start').value || $('#su-start').value.slice(0, 7) + '-01';
    ...
    $('#su-create').onclick = () => {
      ...
      if (!$('#su-helper').value.trim()) { setupError(L('Enter the helper’s name.', '請填寫工人姐姐姓名。'), '#su-helper'); return; }
      if (!$('#su-wage').checkValidity() || !$('#su-wage').value) { setupError(L('Enter the current monthly wage.', '請填寫目前月薪。'), '#su-wage'); return; }
      const exactStart = $('#su-contract-start').value;
      if (exactStart && (!validRecordDate(exactStart) || exactStart > E.todayStr())) {
        setupError(L('Enter a valid first work day no later than today, or leave it blank.', '請填寫不遲於今天的有效首個工作日，或先留空。'), '#su-contract-start'); return;
      }
      ...
      const config = {
        ...
        employmentStartPending: !exactStart,
        employmentStartMonthApprox: exactStart ? exactStart.slice(0, 7) : '',
        profileSchemaVersion: 3,
      ...
      state.config = config;
      const createdFirstProfile = Store.profiles.list().length === 1;
      state.setupDraft = {};
      state.setupSection = 1;
      state.logs = {};
      state.payments = [];
      ...
      toast(exactStart
        ? L('Profile saved. You can record dates from the first work day; no earlier tasks were created.', '檔案已保存。你可由首個工作日起補記；沒有產生較早待辦。')
        : L('Profile saved. You can record this month. Add the first work day later if you need an earlier month.', '檔案已保存。現在可補記本月；如要記錄更早月份，請稍後補填首個工作日。'));
      render();
      if (createdFirstProfile) window.HSReminders?.afterFirstSetup();
    };
  }
  ...
  function bindSettings() {
    ...
    $$('#view [data-switch-helper]').forEach(b => {
      b.onclick = () => {
        ...
        loadActiveProfile();
        toast(L('Switched to ', '已切換至') + (state.config && state.config.helperName || L('helper', '工人姐姐')));
        render();
      };
    });
