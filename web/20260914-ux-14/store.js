 ...
 * own config/logs/payments/adjustments/frozen statements, stored under namespaced keys
...
(function (global) {
  ...
  const PROFILE_KEYS = ['config', 'logs', 'payments', 'adjustments', 'statements'];
  ...
  function migrate() {
    ...
    setRaw(pkey(id, 'statements'), getRaw('statements', []));
    saveProfileList([id]);
    setActiveProfile(id);
  ...
  // Publish a new profile only after its configuration can be stored. Roll
  // back this new profile's keys on failure, never existing payroll records.
  function createProfileWithConfig(config) {
    const id = uid(), key = PREFIX + pkey(id, 'config');
    const registry = localStorage.getItem(PREFIX + 'profiles'), active = localStorage.getItem(PREFIX + 'active');
    const ids = registry === null ? [] : JSON.parse(registry);
    if (!Array.isArray(ids) || ids.some(value => typeof value !== 'string') || new Set(ids).size !== ids.length ||
      ids.includes(id) || localStorage.getItem(key) !== null) throw new Error('Profile registry needs review');
    try {
      setRaw(pkey(id, 'config'), config);
      saveProfileList(ids.concat(id));
      setActiveProfile(id);
      return id;
    } catch (error) {
      let recovered = true;
      for (const [target, previous] of [[key, null], [PREFIX + 'profiles', registry], [PREFIX + 'active', active]]) {
        try {
          if (localStorage.getItem(target) === previous) continue;
          if (previous === null) localStorage.removeItem(target); else localStorage.setItem(target, previous);
        } catch { recovered = false; }
      }
      if (!recovered) throw Object.assign(new Error('Profile creation recovery needs review'), { code: 'setup_recovery_failed' });
      throw error;
    }
  }
  ...
  async function exportAll() {
    ...
    profileList().forEach(id => {
      profiles[id] = {
        ...
        statements: loadFor(id, 'statements', [])
      };
    });
  ...
  async function importAll(jsonText) {
    ...
    if (data.version >= 2) {
      ...
      ids.forEach(id => {
        ...
        setRaw(pkey(id, 'statements'), p.statements || []);
      });
      saveProfileList(ids);
      ...
      setRaw(pkey(id, 'statements'), data.statements || []);
      saveProfileList([id]);
      setActiveProfile(id);
    }
  ...
  const api = {
    migrate,
    profiles: {
      createWithConfig: createProfileWithConfig,
      list: profileList,
    ...
    saveStatements: s => save('statements', s),
    loadUi: () => getRaw('ui', {}),
