    ...
    const issueHtml = (issue, index) => {
      ...
      return '<li><p>' + esc(isZh() ? issue.zh : issue.en) + '</p>' +
        (action.type === 'external' ? '<a class="btn ghost compact" href="' + Compliance.OFFICIAL_CALCULATOR + '" target="_blank" rel="noopener">' +
          ...
          L('Do this next', '先處理這一項') + '</button>') + '</li>';
    };
    const first = assessment.blockers[0], later = assessment.blockers.slice(1);
    return '<div class="legal-status blocked mt"><b>' + L('Next step', '下一步') + '</b><p>' + L('Complete one item at a time. These checks do not stop you recording daily work.',
      '每次只處理一項；這些核對不會阻止你記錄每天的實際情況。') + '</p><ul class="issue-list">' + issueHtml(first, 0) + '</ul>' +
      (later.length ? '<details><summary>' + L('Later: ' + later.length + (later.length === 1 ? ' more check' : ' more checks') + ' before confirming salary',
        '稍後處理：確認薪金前尚有' + later.length + '項') + '</summary><ul class="issue-list">' +
        later.map((issue, index) => issueHtml(issue, index + 1)).join('') + '</ul></details>' : '') +
      '<p class="small mt">' + L('You may still record work and keep a factual record of a payment calculated elsewhere. It will not be labelled as a HelperPay-confirmed calculation.',
