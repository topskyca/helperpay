...
(function () {
  ...
  function setupFormHtml(today, adding) {
    return '<div class="card onboarding-card">' +
      ...
      '<p class="muted small">' + L('Leave this blank if the contract is not at hand. You can add it later without entering old records.', '合約不在手邊可先留空；日後可補填，毋須輸入以往記錄。') + '</p></fieldset>' +
  ...
  function bindSetup() {
    ...
    const employmentStart = () => $('#su-contract-start').value || $('#su-start').value;
    ...
    $('#su-create').onclick = () => {
      ...
      toast(L('Profile saved. Tasks will be created from the tracking date only; no earlier tasks were created.', '檔案已保存。只會由開始追蹤日期起建立待辦，沒有產生較早待辦。'));
      render();
    };
  }
