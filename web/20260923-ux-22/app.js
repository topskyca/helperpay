...
(function () {
  ...
  const APP_VERSION = '1.1.6-web-free';
  ...
  function bindNativeRestore(button, input) {
    ...
    input.onchange = async () => {
      ...
      if (!file || !(await confirmDialog(L('Restore backup?', '還原備份？'),
        L('This replaces local payroll records for every helper. Keep a current backup before continuing.',
          '這會取代所有外傭的本機薪酬記錄。請先保留最新備份。'), L('Restore', '還原'), true))) return;
  ...
  function withPendingMonthAccess(config) {
    if (!config || config.employmentStartPending !== true ||
        Number(config.profileSchemaVersion || 0) >= 3 || !validRecordDate(config.startDate)) return config;
    const createdDay = typeof config.createdAt === 'string' ? config.createdAt.slice(0, 10) : '';
    const trackingDay = validRecordDate(config.checkinView?.startDate) ? config.checkinView.startDate
      : validRecordDate(createdDay) ? createdDay : config.startDate;
    const monthStart = trackingDay.slice(0, 7) + '-01';
    const startDate = config.startDate > monthStart ? monthStart : config.startDate;
    return {
      ...config,
      startDate,
      contractEndDate: Compliance.expectedContractEnd(startDate),
      profileSchemaVersion: 3
    };
  }

  function loadConfigWithPendingMonthAccess() {
    const loaded = Store.loadConfig();
    const upgraded = withPendingMonthAccess(loaded);
    if (upgraded === loaded) return loaded;
    try {
  ...
  const state = {
    config: loadConfigWithPendingMonthAccess(),
    logs: Store.loadLogs(),
    ...
    ui: Object.assign({
      ...
      // HelperPay's current acquisition audience is Hong Kong Chinese. Start a
      // new installation in Traditional Chinese; an existing choice, URL
      // override or the always-visible EN button still wins immediately.
      language: window.HSInitialLanguage || I18n.ZH_HK
    }, Store.loadUi()),
    addingProfile: false,          // true while the "add another helper" setup is open
    onboardingStep: 1,            // retained for old backups; setupSection is the active flow
    setupSection: 1,
    setupJustCompleted: false,
    pendingEmployerDefaults: null, // employer name/PIN carried into a new profile
    setupDraft: {}                 // Preserve entered values across language, reload and back navigation.
  };
  const savedUi = Store.loadUi();
  const savedSetupSection = Number(savedUi.setupSection);
  state.setupSection = savedSetupSection >= 1 && savedSetupSection <= 4 ? savedSetupSection : 1;
  state.setupDraft = savedUi.setupDraft && typeof savedUi.setupDraft === 'object' && !Array.isArray(savedUi.setupDraft)
    ? savedUi.setupDraft : {};
  state.ui.language = Language.pick(location.search, window.HSInitialLanguage || state.ui.language);
  // Mobile-only Firebase improvement data is enabled by default. The native
  // SDKs remain disabled at process start and receive this saved preference
  // only after HelperPay has loaded, so an earlier opt-out remains effective.
  if (state.ui.analyticsProvider !== 'firebase-v2-default') {
    state.ui.analyticsConsent = true;
    state.ui.analyticsProvider = 'firebase-v2-default';
    try { saveUi(); } catch {}
  }
  ...
  function loadActiveProfile() {
    state.config = loadConfigWithPendingMonthAccess();
  ...
  const trackedAnonymousEvents = new Set();
  ...
  function trackAnonymousEvent(name, oncePerLoad = false) {
    if (oncePerLoad && trackedAnonymousEvents.has(name)) return;
    if (oncePerLoad) trackedAnonymousEvents.add(name);
    try { if (typeof window.HSTrack === 'function') window.HSTrack(name); }
    catch { /* Optional analytics must never interrupt payroll work. */ }
  }
  ...
  function saveUi() {
    Store.saveUi({
      ...
      analyticsProvider: state.ui.analyticsProvider || 'firebase-v2-default'
    });
  }
  ...
  function openAbsenceSheet(ds, initialDuration, parent, draftNote) {
    ...
    const dayClass=E.classifyDay(ds,state.config);
    if(dayClass.isRest) {
      toast(L(
        'This is already a rest day, so no leave record is needed. If work occurred, record it on the rest-day screen.',
        '這天已是休息日，毋須再記錄請假。如當日有上班，請在休息日畫面記錄。'
      ));
      return;
    }
    ...
    const ov=openSheet('<h2>'+L('Record leave / absence','記錄請假／缺勤')+'</h2><p>'+esc(fmtDate(ds))+'</p>'+
      ...
      (dayClass.type!=='normal' ? '<p class="form-error">'+L('This date is a holiday. Recording leave will not mark that holiday as taken or move it. Review any overlap separately.',
        '此日期屬假日。記錄請假不會把該假日標為已享用，亦不會搬移日期；重疊情況須另行核對。')+'</p>' : '')+
      ...
      '<p class="muted small">'+L('Saved only on this device. A helper entry remains pending employer review. Use the official calculator or obtain professional advice for pay; this version does not calculate leave entitlements.',
        '只存於本裝置。外傭輸入的記錄仍待僱主核對。薪酬請使用官方計算機或取得專業意見；此版本不計算假期權益。')+'</p><a href="'+officialCalculatorUrl()+'" target="_blank" rel="noopener">'+L('Official entitlement calculator','官方權益計算機')+'</a>'+
  ...
  function workOptions(cls) {
    ...
    const workOption = work => ({ work, label: work === 1 ? L('Worked full day', '上班全日') : L('Worked half day', '上班半日'),
      sub: agreedCash > 0
        ? L('Your recorded extra-cash agreement — not a statutory daily rate', '你已記錄的額外現金協議，並非法定日薪')
        : cls.isRest && cls.restTerms?.restDayWorkArrangement === 'no_extra_payment'
          ? L('No extra cash recorded; the monthly wage is unchanged', '沒有記錄額外現金；月薪不變')
          : L('Check the agreed day-off / compensation arrangement', '請核對已協議的補假／補償安排'),
      amt: agreedCash > 0 ? '+' + money(agreedCash * work) : L('no automatic pay', '不會自動加薪'), cl: agreedCash > 0 ? 'plus' : 'zero' });
    return [
  ...
  function logStatusBadge(entry, cls) {
    if (!entry) return '<span class="badge">' + (cls?.scheduleUnconfirmed
      ? L('Not recorded yet', '尚未記錄')
      : L('Contract default — not yet recorded', '合約預設 — 尚未記錄')) + '</span>';
  ...
  function dayBadges(cls) {
    if (cls.scheduleUnconfirmed) return '<span class="badge pending">' + L('Usual day off not set', '未設定通常休息日') + '</span> ';
    let out = '';
  ...
  function openMissingRestTermsPrompt(ds) {
    const cls = E.classifyDay(ds, state.config);
    if (!cls.isRest || !cls.restTerms?.issue || !(state.logs[ds]?.work > 0) || state.ui.helperMode) return false;
    const ov = openSheet('<h2>' + L('Complete one pay setting', '還差一項薪酬設定') + '</h2><p>' +
      L('Rest-day work is saved, but the app does not yet know whether ordinary rest days are included in the monthly wage or what extra cash / day off was agreed for voluntary work.',
        '休息日上班已保存，但系統還不知道平常休息日是否包括在月薪內，以及自願上班時雙方協議的額外現金／補假安排。') + '</p>' +
      '<div class="legal-status guidance mt"><b>' + L('Why the estimate has not changed', '為何工資估算尚未改變') + '</b><p>' +
      L('HelperPay will not invent a payment. The estimate can include agreed extra cash only after you record the actual arrangement. This reminder will remain on Today and Salary until it is completed.',
        'HelperPay 不會猜測應付金額。記錄實際協議後，估算才可加入協議的額外現金；完成前，「今天」及「薪金」會繼續提醒。') + '</p></div>' +
      '<button class="btn mt" id="rest-terms-now">' + L('Set work and rest arrangements', '設定工作及休息安排') + '</button>' +
      '<button class="btn ghost mt" id="rest-terms-later">' + L('Later — keep the work record', '稍後設定 — 保留工作記錄') + '</button>');
    ov.querySelector('#rest-terms-later').onclick = () => closeSheet(ov);
    ov.querySelector('#rest-terms-now').onclick = () => { closeSheet(ov); openRestHistoryEditor('confirm'); };
    return true;
  }
  ...
  function openDaySheet(ds, initialWork) {
    ...
    const restWorkHint = cls.isRest
      ? '<div class="legal-status mt"><label class="row" style="cursor:pointer;font-size:14px;color:var(--text);font-weight:600"><input id="rest-voluntary" type="checkbox" style="width:auto"' + (entry.restWorkVoluntary !== false ? ' checked' : '') + '><span>' + L('Any work on this rest day was voluntary', '此休息日的任何工作均屬自願') + '</span></label>' +
    ...
    const approveBtn = !state.ui.helperMode && entry.status === 'pending'
      ...
      : '';
    const canAddAbsence = !cls.isRest && (E.isAbsence(entry) || cls.type !== 'normal' || cls.scheduleUnconfirmed);

    const ov = openSheet(
      '<h2>' + esc(fmtDate(ds)) + '</h2>' +
      '<div class="mt">' + dayBadges(cls) + ' ' + logStatusBadge(state.logs[ds], cls) + '</div>' +
      '<p class="muted small mt">' + L('Choose what happened, then press Save. Nothing is recorded until you save.', '選擇當日情況後按「儲存」；儲存前不會改動記錄。') + '</p>' +
      (cls.scheduleUnconfirmed && !entry.dayTypeUnconfirmed ? '<p class="onboarding-note mt">' + L(
        'You can record today now. Set the usual weekly day off later so the calendar and pay review can classify this date.',
        '你可以先記錄今天；稍後設定通常每周休息日，日曆及薪金覆核才會判斷這一天的類型。') + '</p>' : '') +
      ...
      (E.isAbsence(entry) ? '<p class="banner">'+esc(absenceSummary(entry))+'<br>'+L('Choose the correct day status and save to replace this leave record; existing confirmed statements remain unchanged.','請選擇正確的當日情況並儲存，以取代這筆請假記錄；已確認結算仍保留。')+'</p>' : '')+
      '<details class="mt day-optional"><summary>' + (canAddAbsence
        ? L('Add leave details or a note', '加入請假資料或備註')
        : L('Add a note (optional)', '加入備註（選填）')) + '</summary>' +
      (canAddAbsence ? '<button class="btn secondary mt" id="day-absence">'+(E.isAbsence(entry)?L('Check / change leave details','核對／更改請假資料'):L('Record leave / absence details','記錄請假／缺勤資料'))+'</button>' : '')+
      '<label>' + L('Note (optional)', '備註（選填）') + '</label>' +
      '<input id="day-note" placeholder="' + L('e.g. agency confirmed, doctor visit…', '例如：僱傭公司已確認、覆診……') + '" value="' + esc(entry.note || '') + '"></details>' +
      restWorkHint +
      holidayHint +
    ...
    ov.querySelector('#day-save').onclick = () => {
      ...
      closeSheet(ov);
      render();
      if (!openHolidayWorkNext(ds) && !openMissingRestTermsPrompt(ds)) toast(state.ui.helperMode
  ...
  function calculationIssueAction(issue) {
    ...
    if (issue.code === 'absence_average_history') return { type: 'history', dates: issue.relatedDates || [] };
    if (issue.code === 'rest_weekday') return { type: 'guided', guide: 'rest_weekday' };
    if (['rest_day_pay', 'rest_day_work_term', 'rest_history_missing'].includes(issue.code) || issue.code.startsWith('rest_work_amount_')) {
      return { type: 'rest_terms' };
    }
  ...
  function officialCalculatorUrl() {
    return isZh() ? Compliance.OFFICIAL_CALCULATOR_ZH : Compliance.OFFICIAL_CALCULATOR;
  }

  function calculationChecklistHtml(assessment, options = {}) {
    const issueHtml = (issue, index) => {
      const action = calculationIssueAction(issue);
      const historyDates = action.type === 'history' ? action.dates.map(date => {
        const entry = state.logs[date];
        const summary = entry?.absence ? L('Leave recorded', '已記錄請假') :
          entry?.work === 0 ? L('No work recorded', '已記錄不上班') : L('Partial work recorded', '已記錄部分時間上班');
        return '<li>' + esc(fmtDateShort(date)) + ' · ' + summary + '</li>';
      }).join('') : '';
      return '<li><p>' + esc(isZh() ? issue.zh : issue.en) + '</p>' +
        (historyDates ? '<p class="muted small">' + L('Related saved records — no change needed', '相關已保存記錄 — 無須修改') + '</p>' +
          '<ul class="history-date-list">' + historyDates + '</ul><p class="muted small">' +
          L('The statutory average needs a separate calculation. You can record the actual payment calculated separately below.',
            '法定平均工資須另行核算；可於下方記錄另行核算後的實際付款。') + '</p>' +
          '<a class="btn ghost compact" href="' + officialCalculatorUrl() + '" target="_blank" rel="noopener">' +
          L('Labour Department entitlement calculator', '勞工處僱傭權益計算機') + '</a>' :
          action.type === 'external' ? '<a class="btn ghost compact" href="' + officialCalculatorUrl() + '" target="_blank" rel="noopener">' +
          L('View Labour Department calculator', '查看勞工處計算機') + '</a>' :
          '<button type="button" class="btn ghost compact" data-resolve-issue="' + index + '">' +
          L('Review this item', '核對這一項') + '</button>') + '</li>';
    };
    const count = assessment.blockers.length;
    const historyCount = assessment.blockers.filter(issue => calculationIssueAction(issue).type === 'history').length;
    const checkCount = count - historyCount;
    const heading = historyCount ? L(
      (checkCount ? checkCount + (checkCount === 1 ? ' item to check; ' : ' items to check; ') : '') + historyCount +
        (historyCount === 1 ? ' statutory calculation to do separately' : ' statutory calculations to do separately'),
      (checkCount ? checkCount + ' 項記錄／設定待核對；' : '') + historyCount + ' 項法定計算須另行核算') :
      L(count + (count === 1 ? ' item to review before confirming pay' : ' items to review before confirming pay'),
        '確認薪金前須核對 ' + count + ' 項');
    if (options.compact) {
      const available = options.estimateAvailable !== false;
      return '<div class="legal-status ' + (available ? 'guidance' : 'blocked') + ' mt"><b>' +
        (available ? L('Your monthly estimate is available', '本月參考估算已顯示') :
          L('More information is needed for this month', '本月需要補充資料')) + '</b><p>' +
        (available ? L('Keep recording work as usual. Complete these details only before creating a final salary statement.',
          '可照常記錄工作；只需在建立正式薪金結算前補充以下資料。') :
          L('Your saved records are unchanged. Open the details to see what is needed before an estimate can be shown.',
            '已保存的記錄不受影響。展開詳情即可查看顯示估算前所需資料。')) + '</p><details><summary>' +
        L('Details for final statement', '正式結算所需資料') + ' (' + count + ')</summary><ul class="issue-list">' +
        assessment.blockers.map(issueHtml).join('') + '</ul></details></div>';
    }
    return '<div class="legal-status blocked mt"><b>' + heading + '</b><p>' + L('See the items below. You can continue recording work while these checks remain open.',
      '請查看以下項目。期間仍可繼續記錄工作。') + '</p><ul class="issue-list">' +
      assessment.blockers.map(issueHtml).join('') + '</ul></div>';
  }
  ...
  function render() {
    ...
    if (!state.config || state.addingProfile) {
      trackAnonymousEvent('onboarding_started', true);
      trackAnonymousEvent('setup_step_' + state.setupSection + '_view', true);
    ...
    if (state.setupJustCompleted) {
      trackAnonymousEvent('salary_estimate_view', true);
      view.innerHTML = setupSuccessHtml();
      bindSetupSuccess();
      bindLanguageControls();
      window.scrollTo(0, 0);
      return;
    ...
    if (state.ui.view === 'today') trackAnonymousEvent('home_view', true);
    ...
    if (recommendedIndex >= 0 && !state.ui.helperMode && state.ui.view === 'today') {
      const recommended = setupPending[recommendedIndex];
      const later = setupPending.map((item, index) => ({ item, index })).filter(row => row.index !== recommendedIndex);
      const setupMarkup = '<section class="card progressive-setup" id="setup-pending"><p class="eyebrow">' + L('Set up as you go', '逐步完成設定') + '</p>' +
        '<h2>' + esc(isZh() ? recommended.zh : recommended.en) + '</h2><p>' +
        L('This is the only suggested setup step now. You can keep recording even if you do it later.', '現在只建議設定這一項；即使稍後才處理，你仍可繼續記錄。') + '</p>' +
        '<button class="btn secondary" data-resolve-issue="' + recommendedIndex + '">' + L('Set this now', '現在設定') + '</button>' +
  ...
  function renderHeader() {
    ...
    const helperSwitch = $('#helper-switch');
    ...
    language.dataset.language = isZh() ? 'en' : 'zh-HK';
    language.textContent = isZh() ? 'EN' : '繁中';
    language.setAttribute('aria-label', isZh() ? 'Switch to English' : '切換至香港繁體中文');
    if (!state.config || state.addingProfile) {
      chip.style.display = 'none';
      helperSwitch.style.display = 'none';
      return;
    }
    if (multi) {
      helperSwitch.style.display = '';
      helperSwitch.textContent = (state.config.helperName || L('Helper', '工人姐姐')) + ' ▾';
      helperSwitch.setAttribute('aria-label', L('Switch helper. Current: ', '切換工人姐姐。目前：') + (state.config.helperName || L('Helper', '工人姐姐')));
    } else helperSwitch.style.display = 'none';
    chip.style.display = '';
    chip.className = 'mode-chip' + (state.ui.helperMode ? ' helper' : '');
    const roleIcon = state.ui.helperMode ? '👩' : '🔑';
    const roleLabel = state.ui.helperMode
      ? (state.config.helperName || L('Helper', '工人姐姐'))
      : L('Employer', '僱主');
    chip.setAttribute('aria-label', roleLabel);
    chip.innerHTML = '<span class="mode-chip-icon" aria-hidden="true">' + roleIcon + '</span>' +
      '<span class="mode-chip-label">' + esc(roleLabel) + '</span>';
  }

  function switchActiveHelper(id) {
    if (!Store.profiles.list().includes(id) || id === Store.profiles.active()) return;
    Store.profiles.setActive(id);
    loadActiveProfile();
    toast(L('Switched to ', '已切換至') + (state.config?.helperName || L('helper', '工人姐姐')));
    render();
  }

  function beginAddingHelper() {
    if (!state.config || state.ui.helperMode) return;
    state.pendingEmployerDefaults = { employerName: state.config.employerName, employerPin: state.config.employerPin };
    state.addingProfile = true;
    state.setupJustCompleted = false;
    state.setupSection = 1;
    state.setupDraft = {};
    try { saveUi(); } catch { /* The setup form will report an authoritative write failure on save. */ }
    render();
  }

  function openHelperSwitcher() {
    if (!state.config || state.ui.helperMode) return;
    const ids = Store.profiles.list();
    if (ids.length < 2) { beginAddingHelper(); return; }
    HSTrack('helper_switcher_open');
    const active = Store.profiles.active();
    const sheet = openSheet('<h2>' + L('Choose a helper', '選擇工人姐姐') + '</h2><p>' +
      L('Every helper keeps a separate calendar, salary and payment history.', '每位工人姐姐都有獨立的日曆、薪金及付款記錄。') + '</p>' +
      '<div class="helper-switch-list">' + ids.map(id => {
        const config = Store.profiles.loadFor(id, 'config', null);
        const name = config?.helperName || L('My helper', '工人姐姐');
        return '<button class="btn ' + (id === active ? '' : 'secondary') + '" data-header-helper="' + esc(id) + '"' + (id === active ? ' disabled' : '') + '>' +
          esc(name) + (id === active ? L(' · Current', ' · 目前使用中') : '') + '</button>';
      }).join('') + '</div>' +
      '<button class="btn ghost mt" id="header-add-helper">＋ ' + L('Add another helper', '加入另一位工人姐姐') + '</button>' +
      '<button class="btn ghost mt" id="header-helper-close">' + L('Close', '關閉') + '</button>');
    sheet.querySelectorAll('[data-header-helper]').forEach(button => button.onclick = () => {
      const id = button.dataset.headerHelper; closeSheet(sheet); switchActiveHelper(id);
    });
    sheet.querySelector('#header-add-helper').onclick = () => { closeSheet(sheet); HSTrack('add_helper_started'); beginAddingHelper(); };
    sheet.querySelector('#header-helper-close').onclick = () => closeSheet(sheet);
  }

  function renderTabbar() {
    const bar = $('#tabbar');
    if (!state.config || state.addingProfile || state.setupJustCompleted) { bar.style.display = 'none'; return; }
    bar.style.display = '';
    const tabs = [
  ...
  function workedRestDaysMissingTerms(config = state.config, logs = state.logs) {
    if (!config) return [];
    return Object.entries(logs || {}).filter(([date, entry]) => {
      if (!(entry?.work > 0) || !validRecordDate(date)) return false;
      const cls = E.classifyDay(date, config);
      return cls.isRest && !!cls.restTerms?.issue;
    }).map(([date]) => date).sort();
  }
  ...
  function initialSetupPending(config) {
    ...
    if (config.employmentStartPending === true) add('contract_start', 'Add the exact first work day when you need to record an earlier month, calculate the first month, or check the contract end', '如要補記更早月份、計算首月薪金或核對合約完結日，請填寫準確首個工作日');
    if (!PayHistory.complete(config)) {
      const basicsKnown = !!currentPayBasics(config);
      add('effective_terms', basicsKnown
        ? 'Only when finalising a salary statement: confirm whether today’s wage and food arrangement changed during this contract'
        : 'Add the current monthly wage and food arrangement to see an estimate', basicsKnown
        ? '只在確認薪金結算單時：核對目前月薪及膳食安排在本合約期間有否更改'
        : '填寫目前月薪及膳食安排，以顯示估算', basicsKnown ? 'later' : (weekdayKnown ? 'recommended' : 'later'));
    }
    if (weekdayKnown && PayHistory.restWindow(config, config.startDate, config.contractEndDate).issue) {
      const recordedWork = workedRestDaysMissingTerms(config, state.logs).length;
      add('rest_history_missing', recordedWork
        ? recordedWork + ' worked rest day' + (recordedWork === 1 ? ' is' : 's are') + ' recorded. Set the pay arrangement so the estimate can include any agreed extra cash.'
        : 'When needed for salary: check whether rest days are paid and the agreed arrangement if one is worked', recordedWork
        ? '已記錄 ' + recordedWork + ' 個休息日上班。請設定薪酬安排，估算才可加入雙方協議的額外現金。'
        : '需要核對薪金時：確認休息日是否有薪，以及休息日上班的實際協議', recordedWork ? 'recommended' : 'later');
    }
    ...
    if (!['christmas','winter_solstice'].includes(config.winterHolidayChoice)) add('winter_choice', 'Before December: record the employer’s choice of Winter Solstice or Christmas Day. The official Winter Solstice date is 22 Dec in 2026 and 2027.', '12月前：記錄僱主選擇冬節還是聖誕節。勞工處公布的2026及2027年冬節日期均為12月22日。');
    return items;
  }

  function setupProgressHtml(step) {
    return '<div class="setup-progress" aria-label="' + L('Setup progress', '設定進度') + '">' +
      '<div class="spread"><b>' + L('Step ', '第 ') + step + L(' of 4', ' 步，共 4 步') + '</b><span>' + L('About 1 minute', '約需 1 分鐘') + '</span></div>' +
      '<div class="setup-progress-track"><span style="width:' + (step * 25) + '%"></span></div></div>';
  }

  function setupDraftValue(id, fallback = '') {
    return esc(Object.prototype.hasOwnProperty.call(state.setupDraft, id) ? state.setupDraft[id] : fallback);
  }

  function setupFormHtml(today, adding) {
    const step = state.setupSection;
    const headings = {
      1: [adding ? 'Who is the next helper?' : 'What do you call your helper?', adding ? '如何稱呼另一位工人姐姐？' : '你如何稱呼工人姐姐？'],
      2: ['Which day does she usually rest?', '工人姐姐通常星期幾休息？'],
      3: ['What is her current monthly wage?', '工人姐姐目前的月薪是多少？'],
      4: ['Do you provide food free of charge?', '家中是否免費提供膳食？']
    };
    const descriptions = {
      1: L('A first name or familiar nickname is enough. It stays with the records on this device.', '填寫名字或平時使用的稱呼即可，資料會跟隨本機記錄保存。'),
      2: L('Sunday is the common choice and is selected for you. Change it only if the usual arrangement is different.', '已為你選擇常見的星期日；只有通常安排不同才需要更改。'),
      3: L('Enter only the amount paid now. You do not need to reconstruct earlier wages.', '只需填寫目前金額，毋須補錄過往工資。'),
      4: L('Choose the usual arrangement. If an allowance is paid, enter the current monthly amount.', '請選擇平常安排；如支付膳食津貼，再填寫目前每月金額。')
    };
    let fields = '';
    if (step === 1) fields =
      '<label for="su-helper">' + L('Name or nickname', '姓名或稱呼') + '</label><input id="su-helper" autocomplete="name" value="' + setupDraftValue('su-helper') + '" placeholder="' + L('e.g. Maria', '例如：Maria') + '">' +
      '<p class="muted small">' + L('You may leave this blank and use “My helper”. Add a name later if needed.', '可以先留空並使用「工人姐姐」，稍後再補上稱呼。') + '</p>' +
      '<details class="optional-setup mt"><summary>' + L('Try a demo or restore a backup', '體驗示範或還原備份') + '</summary>' +
      ...
      '<p class="muted small mt">' + L('Moving phones or browsers? Restore your payroll backup here. This web version remains free.',
        '更換手機或瀏覽器？可在此還原薪酬備份。此網頁版繼續免費。') + '</p></details>';
    if (step === 2) {
      const rest = setupDraftValue('su-rest', '0');
      fields = '<label for="su-rest">' + L('Usual day off', '通常休息日') + '</label><select id="su-rest" required>' +
        weekdays().map((w, i) => '<option value="' + i + '"' + (String(i) === rest ? ' selected' : '') + '>' + w + '</option>').join('') + '</select>' +
        '<p class="setup-default">✓ ' + L('Sunday is selected. Normal rest days do not need to be logged.', '已預設星期日；如常休息毋須逐日記錄。') + '</p>';
    }
    if (step === 3) fields =
      '<label for="su-wage">' + L('Current monthly wage (HK$)', '目前月薪（港幣）') + '</label><input id="su-wage" type="number" inputmode="decimal" min="0.01" step="0.01" required value="' + setupDraftValue('su-wage') + '" placeholder="5100">' +
      '<p class="muted small">' + L('HelperPay uses this amount to show this month’s starting estimate. Earlier months can be added later if you need them.', 'HelperPay 會用這個金額顯示本月起步估算；如有需要，可稍後再補充較早月份。') + '</p>';
    if (step === 4) {
      const mode = setupDraftValue('su-food-mode', 'provided');
      fields = '<fieldset class="setup-choice-group"><legend>' + L('Food arrangement', '膳食安排') + '</legend>' +
        '<label class="setup-choice"><input id="su-food-provided" type="radio" name="su-food-mode" value="provided"' + (mode === 'provided' ? ' checked' : '') + '><span><b>' + L('Yes — food is provided', '是 — 家中提供膳食') + '</b><small>' + L('No separate monthly food allowance', '不另付每月膳食津貼') + '</small></span></label>' +
        '<label class="setup-choice"><input id="su-food-allowance" type="radio" name="su-food-mode" value="allowance"' + (mode === 'allowance' ? ' checked' : '') + '><span><b>' + L('No — a food allowance is paid', '否 — 每月支付膳食津貼') + '</b><small>' + L('Enter the current amount below', '請在下方填寫目前金額') + '</small></span></label></fieldset>' +
        '<div id="su-food-wrap"' + (mode === 'allowance' ? '' : ' hidden') + '><label for="su-food">' + L('Monthly food allowance (HK$)', '每月膳食津貼（港幣）') + '</label><input id="su-food" type="number" inputmode="decimal" min="0" step="0.01" value="' + setupDraftValue('su-food') + '"></div>' +
        '<div class="onboarding-note mt">' + L(
        'HelperPay collects limited app-use and crash information by default through Google Firebase so we can understand usage and fix problems. It never sends helper or employer names, wages, dates, notes, payment content or screenshots. Advertising tracking is disabled. You can turn reporting off later under Settings → Privacy controls. ',
        'HelperPay 預設透過 Google Firebase 收集有限的 App 使用及崩潰資料，協助我們了解使用情況及修正問題。工人姐姐或僱主姓名、薪金、日期、備註、付款內容及截圖絕不會傳送，廣告追蹤亦已停用。你可稍後在「設定 → 私隱控制」關閉報告。 '
      ) + '<a href="' + pageLink('privacy.html') + '" target="_blank" rel="noopener">' + L('Privacy details', '私隱詳情') + '</a></div>';
    }
    return '<div class="card onboarding-card">' + setupProgressHtml(step) +
      '<div class="onboarding-heading"><p class="eyebrow">' + (adding ? L('Another helper', '加入另一位工人姐姐') : L('Quick setup', '快速設定')) + '</p>' +
      '<h1>' + L(...headings[step]) + '</h1><p>' + descriptions[step] + '</p></div>' +
      '<p id="setup-section-status" class="sr-only" role="status">' + L('Step ', '第 ') + step + L(' of 4', ' 步，共 4 步') + '</p>' +
      '<div class="setup-section" data-setup-section="' + step + '">' + fields + '</div>' +
      '<p id="setup-error" class="form-error" role="alert" hidden></p>' +
      '<div class="setup-buttons">' +
      (step > 1 ? '<button class="btn ghost" id="su-back">' + L('Back', '上一步') + '</button>' : '') +
      (step < 4
        ? '<button class="btn" id="su-next">' + L('Continue', '繼續') + '</button>'
        : '<button class="btn" id="su-create">' + L('Finish and see this month’s pay', '完成並查看本月薪金') + '</button>') +
      '</div>' +
      (adding ? '<button class="btn ghost mt" id="su-cancel">' + L('Cancel adding this helper', '取消加入這位工人姐姐') + '</button>' : '') +
      (step === 1 && !adding ? '<p class="muted small mt">' + L('Language can be changed at any time from the top right. Questions?', '可隨時在右上角切換語言。如有問題，請聯絡：') + ' <a href="' + whatsappUrl() + '" target="_blank" rel="noopener" style="color:var(--accent)">WhatsApp ' + WHATSAPP_DISPLAY + '</a></p>' : '') +
      '</div>';
  }

  function setupHtml() {
    const today = E.todayStr();
    return setupFormHtml(today, state.addingProfile);
  }
  ...
  function bindSetup() {
    ...
    if (cancel) cancel.onclick = async () => {
      if (Object.keys(state.setupDraft).length && !(await confirmDialog(L('Discard setup?', '捨棄設定？'),
        L('The details entered for this new helper have not been saved.', '這位新工人姐姐的已填資料尚未儲存。'), L('Discard draft', '捨棄草稿')))) return;
      state.addingProfile = false;
      state.setupDraft = {};
      state.setupSection = 1;
      state.pendingEmployerDefaults = null;
    ...
    $$('#view input[id^="su-"], #view select[id^="su-"], #view input[name="su-food-mode"]').forEach(field => {
      const label = field.previousElementSibling;
      if (label?.tagName === 'LABEL') label.htmlFor = field.id;
      const draftKey = field.name === 'su-food-mode' ? 'su-food-mode' : field.id;
      const remember = () => {
        if (field.type !== 'radio' || field.checked) state.setupDraft[draftKey] = field.value;
        field.removeAttribute('aria-invalid');
        try { saveUi(); } catch { /* A final save error is still shown before profile creation. */ }
      };
      field.addEventListener('input', remember);
      field.addEventListener('change', () => { remember(); updateSetupFields(); });
    });
    function updateSetupFields() {
      const selected = $('#view input[name="su-food-mode"]:checked');
      const wrap = $('#su-food-wrap');
      if (!wrap) return;
      const shown = selected?.value === 'allowance';
      wrap.hidden = !shown;
      const field = $('#su-food'); field.disabled = !shown; field.required = shown;
    }
    updateSetupFields();
    ...
    const today = E.todayStr();
    const employmentStart = () => today.slice(0, 7) + '-01';
    const readInitialPay = () => PayHistory.prepareInitial({ startDate: employmentStart(),
      contractEndDate: Compliance.expectedContractEnd(employmentStart()), contractSignedOn: '',
      monthlyWage: state.setupDraft['su-wage'], foodMode: state.setupDraft['su-food-mode'] || 'provided', foodAllowance: state.setupDraft['su-food'] || '' },
    ...
    const nextEvents = ['setup_name_next', 'setup_rest_next', 'setup_wage_next'];
    const errorEvents = ['setup_name_error', 'setup_rest_error', 'setup_wage_error', 'setup_food_error'];
    const validateStep = () => {
      if (state.setupSection === 1) {
        const name = ($('#su-helper')?.value || '').trim();
        state.setupDraft['su-helper'] = name;
        if (state.addingProfile && !name) { setupError(L('Enter a name or nickname so the profiles are easy to tell apart.', '請填寫姓名或稱呼，方便分辨不同檔案。'), '#su-helper'); return false; }
        if (name && Store.profiles.list().some(id => String(Store.profiles.loadFor(id, 'config', null)?.helperName || '').trim().toLowerCase() === name.toLowerCase())) {
          setupError(L('This name is already used. Choose a different name or nickname.', '此稱呼已在使用，請填寫另一個姓名或稱呼。'), '#su-helper'); return false;
        }
      }
      if (state.setupSection === 2) {
        const value = $('#su-rest')?.value;
        if (!['0','1','2','3','4','5','6'].includes(value)) { setupError(L('Choose the usual day off.', '請選擇通常休息日。'), '#su-rest'); return false; }
        state.setupDraft['su-rest'] = value;
      }
      if (state.setupSection === 3) {
        const field = $('#su-wage');
        if (!field?.checkValidity() || !field.value || Number(field.value) <= 0) { setupError(L('Enter the current monthly wage.', '請填寫目前月薪。'), '#su-wage'); return false; }
        state.setupDraft['su-wage'] = field.value;
      }
      if (state.setupSection === 4) {
        const mode = $('#view input[name="su-food-mode"]:checked')?.value || 'provided';
        state.setupDraft['su-food-mode'] = mode;
        if (mode === 'allowance') {
          const field = $('#su-food');
          if (!field?.checkValidity() || field.value === '') { setupError(L('Enter the current monthly food allowance.', '請填寫目前每月膳食津貼。'), '#su-food'); return false; }
          state.setupDraft['su-food'] = field.value;
        } else state.setupDraft['su-food'] = '';
      }
      try { saveUi(); } catch { /* Profile creation still performs the authoritative storage write. */ }
      return true;
    };
    const next = $('#su-next');
    if (next) next.onclick = () => {
      if (!validateStep()) { HSTrack(errorEvents[state.setupSection - 1]); return; }
      HSTrack(nextEvents[state.setupSection - 1]);
      state.setupSection += 1; saveUi(); render();
    };
    const back = $('#su-back');
    if (back) back.onclick = () => { HSTrack('setup_back'); state.setupSection = Math.max(1, state.setupSection - 1); saveUi(); render(); };

    const create = $('#su-create');
    if (!create) return;
    create.onclick = () => {
      const button = $('#su-create');
      if (button.disabled || state.setupStorageRecoveryNeeded) return;
      if (!validateStep()) { HSTrack('setup_food_error'); return; }
      HSTrack('setup_food_submit');
      ...
      const restWeekday = state.setupDraft['su-rest'] || '0';
      if (!['0','1','2','3','4','5','6'].includes(restWeekday)) { setupError(L('Choose the usual day off.', '請選擇通常休息日。'), '#su-rest'); return; }
      const restAmount = 0;
      const defaults = state.pendingEmployerDefaults || {};
      const config = {
        ...initialPay, initialSetupVersion: 1,
        checkinView: { version: 1, startDate: today },
        historyTrackingPolicy: 'from_tracking_start',
        employmentStartPending: true,
        employmentStartMonthApprox: '',
        profileSchemaVersion: 3,
        helperName: state.setupDraft['su-helper']?.trim() || L('My helper', '工人姐姐'),
      ...
      state.pendingEmployerDefaults = null;
      state.ui.view = 'today';
      state.setupJustCompleted = true;
      try { saveUi(); } catch { /* The profile itself has already been saved safely. */ }
      try {
        HSTrack('setup_completed');
        if (Store.profiles.list().length > 1) HSTrack('second_helper_completed');
      } catch { /* Optional analytics do not undo a saved profile. */ }
      render();
    };
  }

  function setupSuccessHtml() {
    const day = E.parseYmd(E.todayStr());
    const stmt = E.computeMonth(day.y, day.m, state.config, state.logs);
    const basics = currentPayBasics(state.config);
    const reliable = stmt && !stmt.estimateUnavailable && Number.isFinite(stmt.total);
    const estimate = reliable ? stmt.total : basics?.total;
    return '<section class="card setup-success-card">' +
      '<div class="setup-success-icon" aria-hidden="true">✓</div><p class="eyebrow">' + L('Ready to use', '設定完成') + '</p>' +
      '<h1>' + esc(state.config.helperName || L('Your helper', '工人姐姐')) + ' · ' + L('Estimated pay this month', '本月預計薪金') + '</h1>' +
      (Number.isFinite(estimate) ? '<p class="salary-hero-amount">' + money(estimate) + '</p><p>' + (reliable
        ? L('Updated from the records saved so far.', '已按目前保存的記錄更新。')
        : L('A starting estimate based on the current wage and food arrangement. Record exceptions and complete any requested checks to refine it.', '這是按目前月薪及膳食安排得出的起步估算；記錄例外情況並按提示補充資料後，估算會再更新。')) + '</p>'
        : '<p>' + L('Your profile is ready. Add the requested detail later to complete a supported estimate.', '檔案已準備好；稍後按提示補充所需資料，即可完成受支援的估算。') + '</p>') +
      '<div class="setup-next"><b>' + L('From now on, record only exceptions', '日後只需記錄例外情況') + '</b><p>' +
      L('Normal working days and normal days off need no entry. Record rest-day or holiday work, leave, shortened days and payments.', '一般工作日及如常休息毋須記錄；只需填寫休息日或假日上班、請假、提早下班及付款。') + '</p></div>' +
      '<button class="btn mt" id="setup-start-using">' + L('Go to Today', '進入「今日」') + '</button></section>' +
      '<section class="card setup-reminder-invite" id="setup-reminder-invite"><h2>' + L('Would reminders help?', '需要 HelperPay 提醒你嗎？') + '</h2><p>' +
      L('Get a reminder on rest days, statutory holidays and payday so you do not have to reconstruct the month later.', '在休息日、法定假日及出糧日收到提醒，避免月底才逐日回想。') + '</p>' +
      '<p class="muted small">' + L('Notifications never show names or salary amounts.', '通知不會顯示姓名或薪金。') + '</p>' +
      '<div class="setup-buttons"><button class="btn" id="setup-enable-reminders">' + L('Turn on useful reminders', '開啟建議提醒') + '</button>' +
      '<button class="btn ghost" id="setup-reminder-later">' + L('Not now', '暫時不用') + '</button></div></section>' +
      '<section class="card setup-later"><h2>' + L('You can add details later', '其他資料可稍後加入') + '</h2>' +
      '<button class="btn secondary" id="setup-add-start">' + L('Need an earlier month? Add the first work day', '需要補記以前月份？加入首個工作日') + '</button>' +
      '<button class="btn secondary mt" id="setup-add-helper">' + L('More than one helper at home? Add another helper', '家中有多於一位工人姐姐？加入另一位工人姐姐') + '</button>' +
      '<p class="muted small mt">' + L('Each helper keeps a separate calendar, salary and payment history.', '每位工人姐姐都有獨立的日曆、薪金及付款記錄。') + '</p></section>';
  }

  function bindSetupSuccess() {
    const reminderCard = $('#setup-reminder-invite');
    if (!window.HSReminders?.offerAfterSetup) reminderCard.hidden = true;
    else trackAnonymousEvent('notification_prompt_view', true);
    $('#setup-start-using').onclick = () => { state.setupJustCompleted = false; render(); };
    $('#setup-enable-reminders').onclick = () => {
      HSTrack('notification_enable_tap');
      window.HSReminders?.offerAfterSetup();
    };
    $('#setup-reminder-later').onclick = () => {
      HSTrack('notification_not_now');
      window.HSReminders?.dismissOnboarding();
      reminderCard.remove();
    };
    $('#setup-add-start').onclick = () => {
      state.setupJustCompleted = false; state.ui.view = 'settings'; saveUi(); render();
      openSettingsEditor('contract', '#st-start');
    };
    $('#setup-add-helper').onclick = () => { HSTrack('add_helper_started'); state.setupJustCompleted = false; beginAddingHelper(); };
  }
  ...
  function openQuickRestDaySheet() {
    ...
    const ov = openSheet('<h2>' + L('Set the usual weekly day off', '設定通常每周休息日') + '</h2><p>' +
      L('This prepares the calendar and reminder dates. It does not decide whether the day is paid or what happens if the helper works; those agreement details can be checked later.', '這只會準備日曆及提醒日期，不會代你決定休息日是否有薪，或工人姐姐在休息日上班時如何處理；這些協議資料可稍後核對。') + '</p>' +
  ...
  function currentPayBasics(config) {
    const initial = config.initialPayTerms || {};
    const enteredWage = initial.enteredMonthlyWage;
    const wage = Number(enteredWage ?? config.monthlyWage);
    const foodMode = initial.enteredFoodMode || config.foodMode;
    const enteredFood = initial.enteredFoodAllowance;
    const food = foodMode === 'allowance' ? Number(enteredFood ?? config.foodAllowance) : 0;
    if (!Number.isFinite(wage) || wage <= 0 || !['provided', 'allowance'].includes(foodMode) ||
        !Number.isFinite(food) || food < 0) return null;
    return { wage, foodMode, food, total: wage + food };
  }

  function openBasicProfileSheet() {
      if (!requireMembership()) return;
    if (state.ui.helperMode) return;
    const config = JSON.parse(JSON.stringify(state.config));
    const profileAtOpen = Store.profiles.active(), checksum = Legal.checksum(config);
    const current = currentPayBasics(config);
    const weekday = Number.isInteger(config.restDayWeekday) && config.restDayWeekday >= 0 && config.restDayWeekday <= 6
      ? config.restDayWeekday : 0;
    const ov = openSheet('<h2>' + L('Add the basics', '補充基本資料') + '</h2><p class="muted">' +
      L('This gives you a useful starting estimate now. It does not invent earlier wage history.', '填好便可立即看到實用的起步估算；系統不會猜測以往工資歷史。') + '</p>' +
      '<label for="basic-helper">' + L('Helper’s name', '工人姐姐姓名') + '</label><input id="basic-helper" autocomplete="name" value="' + esc(config.helperName || '') + '">' +
      '<label for="basic-rest">' + L('Usual day off', '通常休息日') + '</label><select id="basic-rest">' +
      weekdays().map((day, index) => '<option value="' + index + '"' + (weekday === index ? ' selected' : '') + '>' + day + '</option>').join('') + '</select>' +
      '<label for="basic-wage">' + L('Current monthly wage (HK$)', '目前月薪（港幣）') + '</label><input id="basic-wage" type="number" inputmode="decimal" min="0.01" step="0.01" value="' + esc(current?.wage || '') + '">' +
      '<label for="basic-food-mode">' + L('Food arrangement', '膳食安排') + '</label><select id="basic-food-mode"><option value="provided"' +
      (current?.foodMode !== 'allowance' ? ' selected' : '') + '>' + L('I provide food free of charge', '我免費提供膳食') + '</option><option value="allowance"' +
      (current?.foodMode === 'allowance' ? ' selected' : '') + '>' + L('I pay a monthly food allowance', '我每月支付膳食津貼') + '</option></select>' +
      '<label for="basic-food">' + L('Monthly food allowance (HK$)', '每月膳食津貼（港幣）') + '</label><input id="basic-food" type="number" inputmode="decimal" min="0" step="0.01" value="' + esc(current?.food || '') + '">' +
      '<p id="basic-error" class="form-error" role="alert" hidden></p><button class="btn mt" id="basic-save">' + L('Save and show my estimate', '保存並顯示估算') + '</button>' +
      '<button class="btn ghost mt" id="basic-cancel">' + L('Cancel', '取消') + '</button>');
    const foodMode = ov.querySelector('#basic-food-mode'), food = ov.querySelector('#basic-food');
    const updateFood = () => { const shown = foodMode.value === 'allowance'; food.hidden = !shown; food.disabled = !shown; food.previousElementSibling.hidden = !shown; };
    foodMode.onchange = updateFood; updateFood();
    const fail = (message, field) => { const box=ov.querySelector('#basic-error');box.hidden=false;box.textContent=message;field?.focus(); };
    ov.querySelector('#basic-cancel').onclick = () => closeSheet(ov);
    ov.querySelector('#basic-save').onclick = () => {
      const name=ov.querySelector('#basic-helper').value.trim(), wage=Number(ov.querySelector('#basic-wage').value),
        rest=Number(ov.querySelector('#basic-rest').value), mode=foodMode.value, allowance=mode==='allowance'?Number(food.value):0;
      if(!name){fail(L('Enter the helper’s name.','請填寫工人姐姐姓名。'),ov.querySelector('#basic-helper'));return;}
      if(!Number.isFinite(wage)||wage<=0){fail(L('Enter the current monthly wage.','請填寫目前月薪。'),ov.querySelector('#basic-wage'));return;}
      if(!Number.isInteger(rest)||rest<0||rest>6){fail(L('Choose the usual day off.','請選擇通常休息日。'),ov.querySelector('#basic-rest'));return;}
      if(!['provided','allowance'].includes(mode)||!Number.isFinite(allowance)||allowance<0){fail(L('Check the food arrangement and allowance.','請核對膳食安排及津貼。'),food);return;}
      let latest;try{latest=Store.loadConfig();}catch{fail(L('Could not read the latest profile. Try again.','未能讀取最新檔案，請重試。'));return;}
      if(Store.profiles.active()!==profileAtOpen||Legal.checksum(latest)!==checksum||Legal.checksum(state.config)!==checksum){fail(L('The helper or settings changed. Close and try again.','外傭或設定已改變，請關閉後重試。'));return;}
      const next={...config,helperName:name,restDayWeekday:rest,monthlyWage:wage,foodMode:mode,foodAllowance:allowance,
        initialPayTerms:{...(config.initialPayTerms||{}),historyChoice:'unknown',knownFrom:null,enteredMonthlyWage:wage,
          enteredFoodMode:mode,enteredFoodAllowance:allowance,recordedAt:new Date().toISOString()}};
      try{Store.saveConfig(next);state.config=next;closeSheet(ov);render();toast(L('Basic details saved.','基本資料已保存。'));}
      catch{fail(L('Could not save. Your original records are unchanged.','未能保存，原有記錄沒有改動。'));}
    };
  }
  ...
  function todayHtml() {
    ...
    } else if (cls.type === 'normal' && !cls.scheduleUnconfirmed && !entry) {
      dayCard = '<div class="card today-routine">' +
        '<div class="spread"><div class="today-date">' + esc(fmtDate(ds)) + '</div></div>' +
        '<div class="today-type">' + dayBadges(cls) + ' <span class="badge normal">' + L('No action needed', '毋須操作') + '</span></div>' +
        '<p class="mt"><b>' + L('Normal working day', '一般工作日') + '</b></p>' +
        '<p class="muted small mt">' + L(
          'Nothing needs to be logged when the helper works as usual. Only record leave, a shortened day or another exception.',
          '工人姐姐如常上班便毋須記錄。只有請假、提早下班或其他例外情況才需要填寫。'
        ) + '</p>' +
        '<button class="btn ghost compact mt" id="today-more">' + L('Record leave or an exception', '記錄請假或例外情況') + '</button>' +
        '</div>';
    } else {
      const recorded = E.validLogWork(entry);
      const statusText = E.isAbsence(entry) || entry?.dayTypeUnconfirmed || (recorded && cls.type==='normal' && entry.work<1) ? describeLogEffect(ds) : recorded ? workOptions(cls).find(option => option.work === entry.work).label : entry ? L('Check this day’s record', '請核對當日記錄') : cls.scheduleUnconfirmed ? L('Did the helper work today?', '工人姐姐今天有上班嗎？') : cls.type === 'normal'
        ? L('Scheduled working day', '按合約預計上班') : L('Scheduled day off', '按合約預計休息／放假');
      dayCard = '<div class="card">' +
        '<div class="spread"><div class="today-date">' + esc(fmtDate(ds)) + '</div></div>' +
        '<div class="today-type">' + dayBadges(cls) + ' ' + logStatusBadge(entry, cls) + '</div>' +
    ...
    if (pend.length) {
      ...
      pendingCard = '<div class="card">' +
        '<h2>' + (state.ui.helperMode
          ...
          : isZh()
            ? '⏳ 待批准的' + esc(state.config.helperName || '工人姐姐') + '記錄（' + pend.length + '）'
    ...
    const basics = currentPayBasics(state.config);
    const reliableEstimate = stmt && !stmt.estimateUnavailable;
    const estimate = reliableEstimate ? stmt.total : basics?.total;
    const estimateHero = '<section class="card salary-hero">' +
      '<p class="eyebrow">' + monthYear(today.y, today.m) + '</p><h1>' + L('Estimated pay this month', '本月預計薪金') + '</h1>' +
      (Number.isFinite(estimate)
        ? '<p class="salary-hero-amount">' + money(estimate) + '</p><p>' + (reliableEstimate
          ? L('Updated from the work and leave records saved so far.', '已按目前保存的上班及請假記錄更新。')
          : L('Starting estimate from the current monthly wage and food arrangement. Salary checks may adjust it.', '這是按目前月薪及膳食安排得出的起步估算；薪金核對後可能調整。')) + '</p>'
        : '<p>' + L('Add the current wage and food arrangement to see an estimate here.', '填寫目前月薪及膳食安排，便可在這裡看到估算。') +
          '</p><button class="btn" id="today-add-basics">' + L('Add basic details', '補充基本資料') + '</button>') +
      (Number.isFinite(estimate) ? '<button class="btn secondary compact mt" id="today-open-salary">' + L('See salary details', '查看薪金詳情') + '</button>' : '') + '</section>';
    let summary = '';
    if (stmt) {
      summary = '<div class="card">' +
        ...
        '<div class="stat"><div class="v">' + (stmt.estimateUnavailable ? (basics ? money(basics.total) : '—') : money(stmt.total)) + '</div><div class="k">' +
        (stmt.estimateUnavailable ? (basics ? L('Starting estimate', '起步估算') : L('Pay calculation needs review', '薪酬計算須核對')) : L('Projected salary', '預計薪金')) + '</div></div>' +
        '</div>' +
        '<p class="muted small mt">' + L(
    ...
    return estimateHero + dayCard + pendingCard + owedCardHtml() + summary + upcoming;
  }

  function bindToday() {
    const basics = $('#today-add-basics');
    if (basics) basics.onclick = () => openBasicProfileSheet();
    const salary = $('#today-open-salary');
    if (salary) salary.onclick = () => { state.ui.view='salary';saveUi();render(); };
  ...
  function pendingEmploymentStartPromptHtml(year, month) {
    const startDate = state.config?.startDate;
    if (state.config?.employmentStartPending !== true || !validRecordDate(startDate) ||
        E.ymd(year, month, 1).slice(0, 7) >= startDate.slice(0, 7)) return '';
    return '<section class="banner mt pending-start-prompt"><h3>' + L('Want to record this earlier month?', '要補記這個較早月份？') + '</h3><p>' +
      L('Earlier dates are unavailable because the first work day was left blank. HelperPay currently starts from ', '由於尚未填寫首個工作日，較早日期暫時無法填寫。HelperPay 目前由 ') +
      '<b>' + esc(fmtDateShort(startDate)) + '</b>' + L('. Add the actual first work day to record earlier dates.', ' 開始記錄；如要補記更早日期，請填寫實際首個工作日。') +
      '</p><button class="btn secondary mt" data-add-employment-start>' + L('Add first work day', '填寫首個工作日') + '</button></section>';
  }
  ...
  function calendarHtml() {
    ...
    return '<div class="card calendar-card">' +
      ...
      pendingEmploymentStartPromptHtml(y, m) +
      '<div class="cal-grid">' +
  ...
  function bindCalendar() {
    ...
    $$('#view [data-add-employment-start]').forEach(button => {
      button.onclick = () => openSettingsEditor('contract', '#st-start');
    });
  ...
  function salaryHtml() {
    ...
    if (!stmt) {
      const pendingPrompt = pendingEmploymentStartPromptHtml(y, m);
      return head + (pendingPrompt || '<p class="muted">' + L('No employment in this month.', '本月不在僱傭期內。') + '</p>') + '</div>';
    }
    ...
    const payBasics = currentPayBasics(state.config);
    const startingEstimate = stmt.estimateUnavailable && payBasics ? payBasics.total : null;
    const restTermsMissingDates = workedRestDaysMissingTerms(state.config, state.logs)
      .filter(date => date >= stmt.periodStart && date <= stmt.periodEnd);
    ...
    let html = head;
    html += '<p class="muted small salary-period">' + esc(fmtDateShort(stmt.periodStart)) + ' – ' + esc(fmtDateShort(stmt.periodEnd)) +
      (stmt.partial ? (isZh() ? ' · 非完整月份（' + stmt.periodDays + '日）' : ' · partial month (' + stmt.periodDays + ' days)') : '') + '</p>';
    if (Number.isFinite(startingEstimate)) html += '<section class="salary-starting-estimate mt"><p class="eyebrow">' +
      L('Starting whole-month estimate', '整月起步估算') + '</p><p class="salary-starting-amount">' + money(startingEstimate) + '</p><p>' +
      L('Based only on the current monthly wage and food arrangement. Add the exact contract date later only if you need a first-month calculation or a final salary statement.',
        '只按目前月薪及膳食安排估算。只有需要計算首月薪金或建立正式結算單時，才需稍後補上準確合約日期。') + '</p></section>';
    if (restTermsMissingDates.length) html += '<section class="legal-status guidance mt" id="salary-rest-terms-reminder"><h2>' +
      L('Rest-day work needs one pay setting', '休息日上班尚欠一項薪酬設定') + '</h2><p>' +
      L('Work is recorded for ', '已記錄以下休息日上班：') + esc(restTermsMissingDates.map(fmtDateShort).join(', ')) + L('. The estimate has not added extra cash because the actual work-and-rest arrangement is not set.', '。由於尚未設定實際工作及休息安排，估算暫未加入額外現金。') + '</p>' +
      '<button class="btn secondary mt" id="salary-rest-terms">' + L('Set this now', '現在設定') + '</button></section>';
    ...
    if (frozen) {
      ...
      html += calculationChecklistHtml(assessment, { compact: true, estimateAvailable: !stmt.estimateUnavailable || Number.isFinite(startingEstimate) });
    ...
    if (!stmt.historicalOnly && !(frozen?.inputs?.recordReview?.employerConfirmed && workReview.ready && !changed)) html += '<section class="legal-status mt" id="salary-work-review"><h2>' +
      (workReview.ready && assessment.ready ? L('Final check before confirming pay', '確認薪金前的最後檢查') :
        L('This month’s work record', '本月工作記錄')) + '</h2>' +
      monthRecordSummaryHtml(workReview) + '<button class="btn secondary mt" id="sal-check-days">' + L('View this month', '查看本月記錄') + '</button></section>';
  ...
  function bindSalary() {
    ...
    const restTermsBtn = $('#salary-rest-terms');
    if (restTermsBtn) restTermsBtn.onclick = () => openRestHistoryEditor('confirm');
  ...
  function settingsFields(section) {
    ...
    const fields={
      people:[['st-helper','helperName',L("Helper's name",'工人姐姐姓名'),'text'],['st-employer','employerName',L("Employer's name (optional)",'僱主姓名（選填）'),'text']],
      ...
      security:[['st-hpin','helperPin',L('Helper PIN (local receipt check)','工人姐姐 PIN（本機收款核對）'),'password'],['st-epin','employerPin',L('Employer PIN (leaving helper mode)','僱主 PIN（離開工人姐姐模式）'),'password']]
    };
  ...
  function openSettingsEditor(section, focusSelector) {
    ...
    const sensitive=section==='contract'||section==='holiday';
    const completingInitialContract=section==='contract'&&config.employmentStartPending===true;
    const requiresAudit=sensitive&&!completingInitialContract;let preview=null;
    const title={people:L('Edit names','修改姓名'),contract:completingInitialContract?L('Add contract dates','補充合約日期'):L('Correct contract dates and type','更正合約日期及類型'),holiday:L('Correct holiday payment terms','更正假日薪酬條款'),security:L('Change local PINs','更改本機 PIN')}[section];
    const ov=openSheet('<h2>'+title+'</h2><p class="muted">'+(requiresAudit?L('Preview the effect before saving. This corrects the recorded contract, not a new contract or a future-dated change. Existing work, holiday arrangements, wage histories, payments and confirmed statements will not be rewritten.',
      ...
      (section==='contract'&&config.employmentStartPending===true?'<p class="banner">'+L('The first work day has not been entered, so earlier months are unavailable. Copy the actual first work day from the signed contract; HelperPay will then open the earlier dates and fill the expected two-year end date.','尚未填寫首個工作日，因此無法補記更早月份。請從已簽合約抄錄實際首個工作日；之後便可填寫較早日期，HelperPay 亦會代你填寫預計兩年合約完結日期。')+'</p>':'')+
      (requiresAudit?'<label for="settings-reason">'+L('Why is this record being corrected?','更正此記錄的原因')+'</label><textarea id="settings-reason"></textarea><div id="settings-preview"></div><button class="btn secondary mt" id="settings-preview-button">'+L('Preview changes','預覽更改')+'</button>':'')+
      '<p class="form-error" role="alert" id="settings-error" hidden></p><button class="btn mt" id="settings-save"'+(requiresAudit?' hidden':'')+'>'+(completingInitialContract?L('Save contract dates','保存合約日期'):L('Save these changes','保存這些更改'))+'</button><button class="btn ghost mt" id="settings-cancel">'+L('Cancel','取消')+'</button>');
    ...
    const invalidate=()=>{preview=null;ov.querySelector('#settings-error').hidden=true;if(requiresAudit){ov.querySelector('#settings-save').hidden=true;ov.querySelector('#settings-preview').innerHTML='';}};
    ov.addEventListener('input',invalidate);ov.addEventListener('change',invalidate);
    ...
    const read=()=>{
      const changes={};for(const row of fields){const value=ov.querySelector('#'+row.id).value.trim();
        ...
        if(row.key==='helperName'&&!value){fail(L('Enter the helper name.','請填寫工人姐姐姓名。'),row.id);return;}
    ...
    if(requiresAudit)ov.querySelector('#settings-preview-button').onclick=()=>{
    ...
    ov.querySelector('#settings-save').onclick=()=>{
      ...
      const next=requiresAudit?preview:read();if(!next){if(requiresAudit)fail(L('Preview the current changes before saving.','保存前請先預覽目前更改。'));return;}
  ...
  function settingsHtml() {
    ...
    if (backupOverdue()) {
      html += '<div class="banner">📥 ' + L(
        'Your records live only on this phone — export a backup below and keep it in your cloud drive.',
        '所有記錄只儲存在這部手機——請在下方匯出備份，並保存到你的雲端硬碟。'
      ) + '</div>';
    }
    ...
    html += '<div class="card" id="settings-profiles"><h2>' + L('Current helper and profiles', '目前工人姐姐及檔案') + '</h2>';
    ids.forEach(id => {
      const cfg = id === activeId ? c : Store.profiles.loadFor(id, 'config', null);
      const name = (cfg && cfg.helperName) || L('My helper', '我的工人姐姐');
      html += '<div class="holiday-row">' +
        '<span class="date">' + esc(name) + '</span>' +
        '<span class="name">' + (cfg ? (cfg.employmentStartPending===true
          ? (cfg.employmentStartMonthApprox?L('Began about ','大約開始於 ')+esc(cfg.employmentStartMonthApprox):L('Employment date can be added later','入職日期可稍後補充'))
    ...
    html += '<button class="btn secondary compact mt" id="add-helper">' + L('+ Add helper', '+ 加入工人姐姐') + '</button>' +
      '<p class="muted small mt">' + L(
        'Each helper has her own calendar, statements, payments and PIN. Switch to the right helper before handing the phone over.',
        '每位工人姐姐都有獨立的日曆、結算單、付款記錄及 PIN。將手機交給工人姐姐前，請先切換至正確的檔案。'
      ) + '</p></div>';
    ...
    html += '<div class="card"><h2>' + L('Statutory holidays', '法定假日') + '</h2>' +
      '<p><b>'+L('Which December statutory holiday did the employer choose?','僱主選擇哪一個12月法定假日？')+'</b><br>'+L('The employer chooses either the Chinese Winter Solstice Festival or Christmas Day. The Winter Solstice date varies by year: it was 21 Dec in 2025 and is 22 Dec in both 2026 and 2027; Christmas Day is 25 Dec. Record the actual choice communicated to the helper.','僱主可選擇冬節或聖誕節。冬節日期每年可能不同：2025年為12月21日，2026及2027年均為12月22日；聖誕節為12月25日。請記錄實際已通知外傭的選擇。')+'</p>'+
      '<p>'+L('Recorded choice: ','目前記錄：')+esc(c.winterHolidayChoice==='winter_solstice'?L('Winter Solstice','冬節'):c.winterHolidayChoice==='christmas'?L('Christmas Day','聖誕節'):L('Not checked yet','尚未核對'))+'</p><button class="btn secondary mt" id="st-winter">'+L('Record the employer’s choice','記錄僱主選擇')+'</button>';
    ...
    html += '<div class="card"><h2>' + L('Data &amp; backup', '資料及備份') + '</h2>' +
      '<p class="muted small">' + L(
        'All records are stored on this device only. A backup includes every helper. ',
        '所有記錄只儲存在此裝置。備份會包括每位外傭的資料。'
      ...
      '<button class="btn ghost compact mt" id="st-reset" style="color:var(--red);border-color:var(--red)">' + L('Erase all data', '清除所有資料') + '</button>' +
      '</div>';
    ...
    html += '<div class="card"><h2>' + L('Privacy controls', '私隱控制') + '</h2>' +
      ...
      '<input type="checkbox" id="st-analytics" style="width:auto"' + (state.ui.analyticsConsent === true ? ' checked' : '') + '>' +
      '<span>' + L('Usage and crash reports', '使用及崩潰報告') + '</span></label>' +
      '<p class="muted small mt">' + L(
        'Enabled by default on the mobile app. Firebase receives general app-use events, an app-installation identifier, approximate location derived from a masked IP address, purchase/subscription events, and technical crash diagnostics. Helper or employer names, wages, calendar dates, notes, payment content and screenshots are never sent. Advertising identifiers and cross-app tracking stay disabled. You can turn this off at any time; restart the app after changing it so the setting fully applies.',
        '手機 App 預設啟用。Firebase 會收到一般使用事件、應用程式安裝識別碼、由遮蔽 IP 位址推算的大概位置、購買／訂閱事件及技術性崩潰診斷；工人姐姐或僱主姓名、薪金、日曆日期、備註、付款內容及截圖絕不會傳送。廣告識別碼及跨 App 追蹤維持停用。你可隨時關閉；更改後請重新開啟 App，讓設定完全生效。'
    ...
    html += '<div class="card"><h2>' + L('Legal calculation boundaries', '法律計算界線') + '</h2>' +
      ...
      '<a class="btn secondary compact mt" href="' + officialCalculatorUrl() + '" target="_blank" rel="noopener" style="text-decoration:none">' + L('Open official entitlement calculator', '開啟官方僱傭權益計算機') + '</a>' +
  ...
  function organizeSettings() {
    ...
    const groups = [
      ...
      ['help', L('Help and calculation limits', '幫助及計算界線'), 'a[href="' + officialCalculatorUrl() + '"]']
    ];
  ...
  function openWinterCalendarSheet(choice) {
    ...
    const ov = openSheet('<h2>' + L('Preview winter holiday dates', '預覽冬季假日日期') + '</h2><p>' +
      L('The employer’s actual choice and the year-specific official dates will be saved together. Other holidays, work records, notes and confirmed statements are kept.', '僱主的實際選擇及該年份的官方日期會一併儲存，其他假日、工作記錄、備註及已確認結算均會保留。') + '</p><ul>' +
    ...
    ov.querySelector('#winter-save').onclick = () => {
      ...
      try {
        const next=HolidayReview.revision(state.config,plan.next,{id:uid(),recordedAt:new Date().toISOString(),reason:L('Confirmed employer winter holiday choice','確認僱主冬季假日選擇')},'winter_choice');
  ...
  function restTermLabel(term) {
    return (weekdays()[term.restDayWeekday] || L('Weekday not checked', '星期待核對')) + ' · ' +
      ...
      (term.restDayWorkArrangement === 'agreed_payment' ? L('Extra cash you recorded: ', '你記錄的額外現金：') + money(term.restDayWorkPayment || 0) + L(' (not a statutory daily rate)', '（並非法定日薪）') :
        term.restDayWorkArrangement === 'no_extra_payment' ? L('No extra cash agreed', '沒有協議額外現金') :
          term.restDayWorkArrangement === 'substituted_rest_day' ? L('Another rest day agreed', '已協議另一天休息') : L('Work arrangement not checked', '工作安排待核對'));
  }
  ...
  function restHistoryHtml(config) {
    ...
    return (!Object.prototype.hasOwnProperty.call(config, 'restTerms') ? '<p class="muted small">' +
      L('These are the rest-day details currently saved. If the original entry was wrong, choose Correct. If the arrangement changed later, choose Change.',
        '以下是目前保存的休息日安排。如一開始填錯，請選擇「更正」；如安排後來真的改變，請選擇「變更」。') + '</p>' : '') +
  ...
  function openRestHistoryEditor(kind) {
    ...
    const currentWage = currentPayBasics(config)?.wage;
    const referenceFullDay = Number.isFinite(currentWage) && currentWage > 0 ? E.round2(E.dailyWage(currentWage)) : null;
    const firstTimeHelp = kind === 'confirm' ? '<div class="onboarding-note"><p><b>' +
      L('Quick example — use only if this matches your real agreement', '快速填寫示例 — 只在符合實際協議時使用') + '</b></p><p>' +
      L('A common arrangement is: ordinary rest days are included in the monthly wage, and voluntary work on a rest day receives separately agreed extra cash. The law does not set one automatic daily rate.',
        '常見安排是：平常休息日包括在月薪內；如外傭自願在休息日上班，另付雙方協議的現金。法例沒有規定自動採用某個日薪。') + '</p>' +
      (referenceFullDay ? '<p>' + L('Reference only from the current monthly wage: full day ', '只按目前月薪提供參考：全日 ') + '<b>' + money(referenceFullDay) + '</b> · ' +
        L('half day ', '半日 ') + '<b>' + money(E.round2(referenceFullDay / 2)) + '</b>. ' + L('Change it if this is not the amount actually agreed.', '如並非實際協議金額，請更改。') + '</p>' : '') +
      '<button type="button" class="btn ghost compact" id="rest-history-example">' + L('Fill this example', '填入這個示例') + '</button></div>' : '';
    const reasonField = kind === 'confirm' ? '' : '<label for="rest-history-reason">' + L('Reason / agreement reference (required)', '原因／協議記錄（必填）') + '</label><textarea id="rest-history-reason"></textarea>';
    const ov = openSheet('<h2>' + (kind === 'change' ? L('Change rest-day terms from a date', '由指定日期起變更休息日條款') : kind === 'confirm' ? L('Check and complete rest-day history', '核對並補齊休息日歷史') : L('Correct rest-day history', '更正休息日歷史')) + '</h2>' +
      ...
      '<div class="legal-status"><p><b>' + L('Two separate questions', '這是兩個不同問題') + '</b></p><p>' + L(
        'First, are scheduled rest days included in the monthly wage? Second, if the helper voluntarily works on one, was any extra cash or another rest day actually agreed? Hong Kong law does not set one automatic day-rate payment for rest-day work.',
        '第一，平常休息日是否已包括在月薪內；第二，如外傭自願在休息日上班，雙方實際有否協議額外現金或另一天休息。香港法例沒有規定休息日上班自動加一天日薪。'
      ) + '</p><p class="small"><a href="https://www.labour.gov.hk/eng/faq/cap57e_whole.htm" target="_blank" rel="noopener">' + L('See the Labour Department rest-day FAQ', '查看勞工處休息日常見問題') + '</a></p></div>' +
      firstTimeHelp + '<div id="rest-history-rows"></div>' + (kind !== 'change' ? '<button class="btn ghost mt" id="rest-history-add">' + L('Add an effective period', '加入生效期間') + '</button>' : '') +
      reasonField +
    ...
    const draw = () => {
      ov.querySelector('#rest-history-rows').innerHTML = draft.map((row, i) => '<fieldset class="card"><legend>' + L('Period ', '期間 ') + (i + 1) + '</legend>' +
        ...
        '<label for="rh-pay-' + i + '">' + L('Are normal rest days included in the monthly wage?', '平常休息日是否已包括在月薪內？') + '</label><select id="rh-pay-' + i + '"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="paid"' + (row.restDayPayTerm === 'paid' ? ' selected' : '') + '>' + L('Yes — paid rest days', '是 — 有薪休息日') + '</option><option value="unpaid"' + (row.restDayPayTerm === 'unpaid' ? ' selected' : '') + '>' + L('No — unpaid rest days', '否 — 無薪休息日') + '</option></select><p class="muted small">' + L('Suggested starting answer: Yes, if this matches how the monthly wage is actually agreed.', '建議起步答案：如符合實際月薪協議，可選「是」。') + '</p>' +
        '<label for="rh-work-' + i + '">' + L('If a rest day is worked voluntarily, what was agreed?', '如自願在休息日上班，雙方有甚麼協議？') + '</label><select id="rh-work-' + i + '"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="agreed_payment"' + (row.restDayWorkArrangement === 'agreed_payment' ? ' selected' : '') + '>' + L('Extra cash amount agreed', '已協議額外現金') + '</option><option value="no_extra_payment"' + (row.restDayWorkArrangement === 'no_extra_payment' ? ' selected' : '') + '>' + L('No extra cash agreed', '沒有協議額外現金') + '</option><option value="substituted_rest_day"' + (row.restDayWorkArrangement === 'substituted_rest_day' ? ' selected' : '') + '>' + L('Another rest day agreed', '已協議另一天休息') + '</option></select><p class="muted small">' + L('Choose the actual agreement. If extra cash was agreed, enter the full-day amount below; a half-day work record uses half of it.', '請按實際協議選擇。如協議另付現金，請在下方填寫全日金額；半日工作記錄會採用一半。') + '</p>' +
        '<div id="rh-amount-row-' + i + '"' + (row.restDayWorkArrangement !== 'agreed_payment' ? ' hidden' : '') + '><label for="rh-amount-' + i + '">' + L('Extra cash actually agreed for a full day (HK$)', '全日工作實際協議的額外現金（港幣）') + '</label><input type="number" inputmode="decimal" step="0.01" id="rh-amount-' + i + '" value="' + esc(row.restDayWorkPayment ?? '') + '"><p class="muted small">' + L('Enter the amount you actually agreed. Do not use the app’s reference daily rate unless that was truly your agreement.', '請填寫雙方實際協議的金額。除非雙方確實如此協議，否則不要照用本程式的參考日率。') + '</p></div>' +
    ...
    const example = ov.querySelector('#rest-history-example');
    if (example) example.onclick = () => {
      capture();
      draft = draft.map(row => ({ ...row, restDayPayTerm: 'paid', restDayWorkArrangement: 'agreed_payment',
        restDayWorkPayment: referenceFullDay || row.restDayWorkPayment || '' }));
      invalidate(); draw(); ov._dirty = true;
    };
  ...
  function bindSettings() {
    ...
    if (analytics) analytics.onchange = () => {
      ...
      toast(analytics.checked
        ? L('Usage and crash reporting enabled — restart the app to fully apply', '已啟用使用及崩潰報告；請重新開啟 App 以完全生效')
        : L('Usage and crash reporting disabled — restart the app to fully apply', '已停用使用及崩潰報告；請重新開啟 App 以完全生效'));
    };
    ...
    $('#add-helper').onclick = () => { HSTrack('add_helper_started'); beginAddingHelper(); };

    $$('#view [data-switch-helper]').forEach(b => {
      b.onclick = () => {
        switchActiveHelper(b.dataset.switchHelper);
      };
    });
    ...
    $$('#view [data-del-helper]').forEach(b => {
      b.onclick = async () => {
        ...
        const name = (cfg && cfg.helperName) || L('this helper', '這位工人姐姐');
        if (!(await confirmDialog(
    ...
    $('#st-reset').onclick = async () => {
      if (!(await confirmDialog(
        ...
        L('All HelperPay records and preferences on this browser will be deleted. Backups saved elsewhere will remain.', '此瀏覽器內所有 HelperPay 記錄及選項將被刪除，另存於其他位置的備份仍會保留。'),
  ...
  async function toggleMode() {
    ...
    if (!state.ui.helperMode) {
      ...
      saveUi();
      toast(L('Helper mode — hand the phone over 👋', '工人姐姐模式 — 可以把手機交給工人姐姐 👋'));
      render();
    } else {
      if (state.config.employerPin) {
        const pin = await pinDialog(L('Exit helper mode', '離開工人姐姐模式'), L('Enter the employer PIN.', '請輸入僱主 PIN。'));
        if (pin === null) return;
  ...
  window.HSWebBoot( () => {
    ...
    $('#helper-switch').onclick = openHelperSwitcher;
    trackAnonymousEvent('app_session_open', true);
    render();
    if (!window.Capacitor?.isNativePlatform() && 'serviceWorker' in navigator && location.protocol.startsWith('http')) {
