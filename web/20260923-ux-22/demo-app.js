...
(function () {
  ...
  function settingsHtml() {
    ...
    html += '<div class="card"><h2>' + L('Privacy controls', '私隱控制') + '</h2>' +
      ...
      '<p class="muted small mt">' + L(
        'Usage statistics and subscription processing are disabled in this demo. Real payroll and subscriptions are not accessed.',
        '此示範模式停用使用統計及訂閱處理，不會存取真實薪酬或訂閱資料。'
  ...
  function bindSettings() {
    ...
    $('#st-reset').onclick = async () => {
      if (!(await confirmDialog(
        ...
        L('Only this demo workspace and its temporary share copies are reset. Real payroll and subscriptions are unchanged. Copies you exported elsewhere remain there.', '只會重設示範工作區及其暫存分享檔案，真實薪酬及訂閱不受影響。已匯出至其他位置的副本仍會保留。'),
