...
(function (global) {
  ...
  const OFFICIAL_CALCULATOR_ZH = 'https://www.labour.gov.hk/tc/labour/Statutory_Employment_Entitlements_Reference_Calculator.htm';

  function item(code, en, zh, section, relatedDates) {
    return Object.freeze({ code, en, zh, section: section || 'settings',
      ...(relatedDates ? { relatedDates: Object.freeze([...relatedDates]) } : {}) });
  }
  ...
  function averageHistoryReviewDates(config, logs, statement, Engine) {
    if (!statement || !Engine) return [];
    const from = Legal.addMonths(statement.periodStart, -12);
    return Object.keys(logs || {}).filter(date => date >= from && date < statement.periodStart &&
      Engine.isEmployedOn(date, config) &&
      (Engine.isAbsence(logs[date]) || (Engine.classifyDay(date, config).type === 'normal' &&
        typeof logs[date]?.work === 'number' && logs[date].work < 1))).sort();
  }
  ...
  function assessMonth(input) {
    ...
    if (restHistory.periods.some(p => !['no_extra_payment', 'agreed_payment', 'substituted_rest_day'].includes(p.restDayWorkArrangement))) {
    ...
    if (config.winterHolidayChoice !== 'winter_solstice' && config.winterHolidayChoice !== 'christmas') {
      blockers.push(item('winter_choice', 'Record the employer’s choice of Winter Solstice or Christmas Day as the statutory holiday.', '請記錄僱主選擇冬節或聖誕節作為法定假日。'));
    }
    ...
    const averageReviewDates = averageHistoryReviewDates(config, logs, statement, Engine);
    if (averageReviewDates.length) {
      blockers.push(item('absence_average_history',
        'The earlier partial-work or no-work records are preserved. They are not marked wrong. Statutory average pay cannot be verified without complete actual paid-wage and excluded-period details; calculate the relevant entitlement separately.',
        '較早前部分上班／不上班的記錄會原樣保留，並非標記為錯誤。系統沒有完整的實際已付工資及須剔除期間資料，無法核實法定平均工資；相關權益須另行核算。',
        'salary', averageReviewDates));
    }
  ...
  const api = {
    ...
    OFFICIAL_CALCULATOR,
    OFFICIAL_CALCULATOR_ZH,
    expectedContractEnd,
    averageHistoryReviewDates,
    officialDates,
    hasAuditedCalendar,
    assessMonth,
    acceptedMoneyItems
