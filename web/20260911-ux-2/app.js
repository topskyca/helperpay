...
(function () {
  ...
  const APP_VERSION = '1.1.2-web-free';
  ...
  function render() {
    ...
    if (setupPending.length && !state.ui.helperMode) {
      view.insertAdjacentHTML('afterbegin', '<section class="card" id="setup-pending"><h2>' + L('Contract details still to check', '合約資料待核對') + '</h2><p>' +
        L('Your profile is saved. You can record facts now; these details are not confirmed for payroll.', '檔案已保存，可以先記錄實際情況；以下資料尚未確認，不會當作已核實計薪條款。') + '</p><details class="mt"><summary>' +
        L('Review ', '查看 ') + setupPending.length + L(' pending items', '項待核對資料') + '</summary><ul class="issue-list">' + setupPending.map((item, index) =>
        '<li><p>' + esc(isZh() ? item.zh : item.en) + '</p><button class="btn ghost compact" data-resolve-issue="' + index + '">' + L('Check this detail', '核對此項') + '</button></li>').join('') + '</ul></details></section>');
  ...
  function initialSetupPending(config) {
    ...
    const items = [], add = (code, en, zh) => items.push({ code, en, zh });
    if (!validRecordDate(config.contractSignedOn)) add('contract_signed_on', 'ID 407 contract signed date', 'ID 407 合約簽署日期');
    if (!PayHistory.complete(config)) add('effective_terms', 'Wage and food history — complete the unknown periods', '工資及膳食歷史 — 補齊未知期間');
    if (PayHistory.restWindow(config, config.startDate, config.contractEndDate).issue) add('rest_history_missing', 'Weekly rest day, pay and agreed work arrangement', '每周休息日、薪酬及已協議的工作安排');
    if (!['paid','unpaid'].includes(config.firstThreeMonthHolidayPayTerm)) add('first_three_months', 'Statutory-holiday pay in the first three months', '首三個月法定假日薪酬');
    if (!['christmas','winter_solstice'].includes(config.winterHolidayChoice)) add('winter_choice', 'Winter Solstice / Christmas choice — neither is assumed', '冬節／聖誕節選擇 — 不會代選任何一日');
    return items;
  }
  ...
  function setupFormHtml(today, adding) {
    const pendingOption = '<option value="unknown">' + L('Not yet agreed / need to check', '尚未商定／需要核對') + '</option>';
    return '<div class="card onboarding-card">' +
      ...
      '<p class="muted small">' + L('Enter the actual first work day. Unknown terms may be saved for later checking; do not guess.', '請填寫真實的首個工作日。未知條款可以保存為待核對，不必猜答案。') + '</p>' +
      '<label>' + L('Helper\'s name (optional)', '外傭姓名（選填）') + '</label><input id="su-helper" autocomplete="name" placeholder="' + L('e.g. Maria', '例如：Maria') + '">' +
      '<label>' + L('ID 407 signed date (may be checked later)', 'ID 407 簽署日期（可稍後核對）') + '</label><input id="su-signed" type="date">' +
      ...
      '<label>' + L('Current monthly wage (HK$; leave blank if unknown)', '目前月薪（港幣；未知可留空）') + '</label><input id="su-wage" type="number" inputmode="decimal" min="0.01" step="0.01">' +
      ...
      '<option value="provided">' + L('Food provided free', '免費提供膳食') + '</option><option value="allowance">' + L('Monthly food allowance', '每月膳食津貼') + '</option>' + pendingOption + '</select>' +
      ...
      '<label for="su-pay-history">' + L('Have wage and food terms been the same since the first work day?', '工資及膳食安排自首個工作日起一直相同嗎？') + '</label><select id="su-pay-history"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="same">' + L('Yes — I have checked both', '是 — 我已核對兩項安排') + '</option><option value="changed">' + L('No — enter when the current terms began', '曾經調整 — 填寫目前安排的生效日期') + '</option><option value="unknown">' + L('I need to check the history later', '我需要稍後核對歷史') + '</option></select>' +
      '<label for="su-pay-from">' + L('Current wage AND food terms apply from', '目前工資及膳食安排同時適用的起始日期') + '</label><input id="su-pay-from" type="date">' +
      '<p class="muted small mt" id="setup-history-note">' + L('Only an explicit “same” answer applies these amounts from the first work day. Earlier periods are left unknown if terms changed. You can complete each period after saving; unknown history cannot be confirmed as a payroll calculation.', '只有明確選擇「一直相同」，才會把此金額用於首個工作日起的期間。若曾調整，較早期間會保留為未知；保存後可逐段補齊。未知歷史不能用作已確認的薪金計算。') + '</p>' +
      ...
      '<label>' + L('Weekly rest day', '每周休息日') + '</label><select id="su-rest"><option value="">' + L('Choose…', '請選擇……') + '</option>' +
      weekdays().map((w, i) => '<option value="' + i + '">' + w + '</option>').join('') +
      '<option value="-1">' + L('Need to check the weekly arrangement', '需要核對每周安排') + '</option></select>' +
      '<label>' + L('Are weekly rest days paid?', '每周休息日是否有薪？') + '</label><select id="su-rest-pay"><option value="">' + L('Choose the agreed term…', '請選擇已協議條款……') + '</option><option value="paid">' + L('Paid', '有薪') + '</option><option value="unpaid">' + L('Unpaid', '無薪') + '</option>' + pendingOption + '</select>' +
      '<label>' + L('If a rest day is worked', '如在休息日工作') + '</label><select id="su-rest-work"><option value="">' + L('Choose the agreed arrangement…', '請選擇已協議安排……') + '</option><option value="substituted_rest_day">' + L('Agreed substituted rest day', '雙方同意補回休息日') + '</option><option value="agreed_payment">' + L('Agreed cash amount', '雙方同意現金金額') + '</option>' + pendingOption + '</select>' +
      ...
      '<label>' + L('Statutory-holiday pay during first 3 months', '首3個月法定假日薪酬') + '</label><select id="su-early-holiday"><option value="">' + L('Choose the contractual term…', '請選擇合約條款……') + '</option><option value="paid">' + L('Paid by agreement', '按協議有薪') + '</option><option value="unpaid">' + L('Unpaid', '無薪') + '</option>' + pendingOption + '</select>' +
      '<label>' + L('Contractual winter statutory holiday', '合約訂明的冬季法定假日') + '</label><select id="su-winter"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="winter_solstice">' + L('Winter Solstice', '冬節') + '</option><option value="christmas">' + L('Christmas Day', '聖誕節') + '</option>' + pendingOption + '</select>' +
      ...
      '<button class="btn mt" id="su-create" hidden>' + L('Save profile and start recording', '保存檔案並開始記錄') + '</button>' +
      ...
      '<p class="muted small mt">' + L(
        'Known statutory holidays are prefilled. If the winter choice is unknown, neither option is assumed. Pending details stay visible after saving and keep the relevant calculations unconfirmed.',
        '系統會預載已知的法定假日；冬季選擇未知時，不會代選任何一日。保存後仍會列出待核對資料，相關計算不可標為已確認。'
      ) + '</p>' +
      (adding ? '' :
  ...
  function setupHtml() {
    ...
    if (state.onboardingStep === 2) {
      return '<div class="card onboarding-card">' +
        ...
        '<div><span>✓</span><p><b>' + L('Contract signed date', '合約簽署日期') + '</b><small>' + L('Unknown details can be checked later; do not guess', '未知資料可稍後核對，不必猜填') + '</small></p></div>' +
        ...
        '<div><span>✓</span><p><b>' + L('Rest-day and holiday agreements', '休息日及假日協議') + '</b><small>' + L('Weekly day, paid/unpaid terms, work compensation and winter choice', '每周日期、有薪／無薪、工作補償及冬季選擇') + '</small></p></div>' +
        '<div><span>✓</span><p><b>' + L('Previous wage / food changes', '過往工資／膳食變更') + '</b><small>' + L('Dates and amounts, or save them as needing review', '日期及金額，或先保存為待核對') + '</small></p></div>' +
        '</div>' +
  ...
  function bindSetup() {
    ...
    $$('#view input[id^="su-"], #view select[id^="su-"]').forEach(field => {
      ...
      field.required = !['su-helper', 'su-signed', 'su-wage'].includes(field.id);
    });
    function updateSetupFields() {
      for (const [id, shown] of [['su-food', $('#su-food-mode').value === 'allowance'],
        ['su-rest-payment', $('#su-rest-work').value === 'agreed_payment'], ['su-pay-from', $('#su-pay-history').value === 'changed']]) {
    ...
    const readInitialPay = () => PayHistory.prepareInitial({ startDate: $('#su-start').value,
      contractEndDate: Compliance.expectedContractEnd($('#su-start').value), contractSignedOn: $('#su-signed').value,
      monthlyWage: $('#su-wage').value, foodMode: $('#su-food-mode').value, foodAllowance: $('#su-food').value },
      $('#su-pay-history').value, $('#su-pay-from').value, { asOf: E.todayStr(), recordedAt: new Date().toISOString() });
    ...
    $('#su-create').onclick = () => {
      ...
      const required = [['#su-rest', ['-1','0','1','2','3','4','5','6'].includes(restWeekday)],
        ['#su-rest-pay', ['paid','unpaid','unknown'].includes(restPay)], ['#su-rest-work', ['agreed_payment','substituted_rest_day','unknown'].includes(restWork)],
        ['#su-early-holiday', ['paid','unpaid','unknown'].includes(earlyHoliday)], ['#su-winter', ['winter_solstice','christmas','unknown'].includes(winterChoice)]];
      ...
      const config = {
        ...
        checkinView: { version: 1, startDate: E.todayStr() },
  ...
  function openSettingsEditor(section, focusSelector) {
    ...
    const ov=openSheet('<h2>'+title+'</h2><p class="muted">'+(sensitive?L('Preview the effect before saving. This corrects the recorded contract, not a new contract or a future-dated change. Existing work, holiday arrangements, wage histories, payments and confirmed statements will not be rewritten.',
      ...
      fields.map(row=>'<label for="'+row.id+'">'+row.label+'</label>'+(row.options?'<select id="'+row.id+'">'+row.options.map(([value,label])=>'<option value="'+esc(value)+'"'+((config[row.key]||'')===value?' selected':'')+'>'+label+'</option>').join('')+'</select>':'<input id="'+row.id+'" type="'+row.type+'"'+(row.type==='password'?' inputmode="numeric" minlength="4" maxlength="6" autocomplete="off"':row.type==='number'?' inputmode="decimal" min="0" step="0.01"':'')+' value="'+esc(config[row.key]??'')+'">')).join('')+
  ...
  function settingsHtml() {
    ...
    ids.forEach(id => {
      ...
      html += '<div class="holiday-row">' +
        ...
        '<span class="name">' + (cfg ? L('First day: ', '首個工作日：') + esc(validRecordDate(cfg.startDate) ? I18n.formatDate(cfg.startDate,state.ui.language) : L('Needs checking','待核對')) : '') + '</span>' +
        (id === activeId
    ...
    html += '<div class="card"><h2>' + L('ID 407 contract dates', 'ID 407 合約日期') + '</h2>' +
    ...
    html += '<div class="card"><h2>' + L('Statutory holidays', '法定假日') + '</h2>' +
      '<p>'+L('Contractual winter choice: ','合約冬季選擇：')+esc(c.winterHolidayChoice==='winter_solstice'?L('Winter Solstice','冬節'):c.winterHolidayChoice==='christmas'?L('Christmas Day','聖誕節'):L('Needs checking','待核對'))+'</p><button class="btn secondary mt" id="st-winter">'+L('Review winter choice — preview first','核對冬季選擇 — 先預覽')+'</button>';
  ...
  function openPayHistoryEditor(kind) {
    ...
    if (!validRecordDate(config.startDate) || !validRecordDate(config.contractEndDate) || !validRecordDate(config.contractSignedOn)) {
      const missingDate = !validRecordDate(config.contractSignedOn) ? 'contract_signed_on' : !validRecordDate(config.startDate) ? 'contract_start' : 'contract_end';
