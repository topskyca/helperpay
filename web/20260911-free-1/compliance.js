...
(function (global) {
  ...
  function hasAuditedCalendar(config, year) {
    ...
    const expected = officialDates(year, config.winterHolidayChoice);
    const actual = new Set((config.holidays || [])
      .filter(day => day.type === 'statutory_holiday' || day.officialId)
      .map(day => day.date));
    return expected.length === 15 && expected.every(date => actual.has(date));
  }
  ...
  function assessMonth(input) {
    ...
    if (config.contractType !== 'ID407') {
      blockers.push(item('contract_type', 'Confirm that this is a Standard Employment Contract (ID 407).', '請確認這是標準僱傭合約（ID 407）。'));
    }
    if (!isDate(config.contractSignedOn)) {
      blockers.push(item('contract_signed_on', 'Enter the date the ID 407 contract was signed.', '請輸入 ID 407 合約簽署日期。'));
    }
    if (!isDate(config.startDate)) {
    ...
    if (!isDate(config.contractEndDate) || config.contractEndDate !== expectedEnd) {
      blockers.push(item('contract_end', 'Confirm the complete two-year ID 407 contract period. Record early termination separately.', '請確認完整的兩年 ID 407 合約期；提早終止須另行記錄。'));
    }
    ...
    if (config.restDayPayTerm !== 'paid' && config.restDayPayTerm !== 'unpaid') {
    ...
    } else if (config.restDayPayTerm === 'unpaid') {
    ...
    if (config.restDayWorkArrangement !== 'agreed_payment' && config.restDayWorkArrangement !== 'substituted_rest_day') {
    ...
    if (isDate(config.contractSignedOn) && isDate(config.startDate) && isDate(config.contractEndDate)) {
      ...
      if (!activeWage || Number(activeWage.monthlyWage) !== Number(config.monthlyWage)) {
    ...
    const deductionLines = (statement.lines || []).filter(line => line.kind === 'deduction');
    if (deductionLines.length) {
      blockers.push(item('absence_calculation', 'This month contains an absence deduction. Exact duration and a lawful proportional maximum must be independently confirmed.', '本月包含缺勤扣款；必須另行確認實際缺勤時間及合法的按比例扣款上限。', 'salary'));
    }
    ...
    if (Engine && isDate(config.startDate)) {
      ...
      Object.keys(logs).filter(date => date >= statement.periodStart && date <= statement.periodEnd).forEach(date => {
        ...
        if (cls.isRest) {
          ...
          if (config.restDayWorkArrangement === 'agreed_payment' && !(Number(config.restDayWorkPayment) > 0)) {
          ...
          if (config.restDayWorkArrangement === 'substituted_rest_day') {
        ...
        if (cls.holiday && (cls.holiday.type === 'statutory_holiday' || cls.holiday.officialId)) {
          ...
          if (!arrangement) {
            ...
            if (errors.length) blockers.push(item('holiday_arrangement_' + date, errors.join(' '), '法定假日安排資料不符合時限或通知規定。', 'today'));
          }
