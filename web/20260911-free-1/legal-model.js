...
(function (global) {
  ...
  const MODEL_VERSION = 'fdh-id407-v2.0.0';
  const LEGAL_SOURCE_VERSION = 'hk-ld-2026-09-08';
  ...
  function hoursBetween(earlier, later) {
    ...
    if (Number.isNaN(start) || Number.isNaN(end)) fail('noticeAt must be a valid ISO date-time');
  ...
  function validateHolidayArrangement(arrangement) {
    ...
    if (arrangement.type === 'alternative_holiday') {
      ...
      if (!arrangement.noticeAt) {
        errors.push('At least 48 hours prior notice is required');
      } else {
        try {
          if (hoursBetween(arrangement.noticeAt, arrangement.statutoryDate + 'T00:00:00.000Z') < 48) {
            errors.push('At least 48 hours prior notice is required');
          }
      ...
      if (distance > 30) errors.push('Substituted holiday must be within 30 days before or after its reference holiday');
  ...
  function validateContractTerms(contract) {
    ...
    (contract.wagePeriods || []).forEach((period, index) => {
      if (!(Number(period.monthlyWage) >= minimum.monthlyWage)) {
    ...
    (contract.foodTerms || []).forEach((period, index) => {
      ...
      } else if (period.mode === 'allowance' && !(Number(period.monthlyAmount) >= minimum.foodAllowance)) {
