...
(function (global) {
  ...
  function computeMonth(year, month, config, logs) {
    ...
    const foodMonthly = config.foodMode === 'provided' ? 0 : (config.foodAllowance || 0);
    ...
    return {
      ...
      food, totalExact, acceptedTotalExact,
  ...
  function statementText(stmt, config, extras) {
    ...
    L.push('Reference rate only (not universal): ' + fmtMoney(config.monthlyWage) + ' × 12 ÷ 365 = HK$' + stmt.dailyWage.toFixed(4));
    L.push('');
    L.push(stmt.partial
