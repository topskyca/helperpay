  ...
  function openRestHistoryEditor(kind) {
    ...
    const draw = () => {
      ov.querySelector('#rest-history-rows').innerHTML = draft.map((row, i) => '<fieldset class="card"><legend>' + L('Period ', '期間 ') + (i + 1) + '</legend>' +
        ...
        '<label for="rh-pay-' + i + '">' + L('Are normal rest days included in the monthly wage?', '平常休息日是否已包括在月薪內？') + '</label><select id="rh-pay-' + i + '"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="paid"' + (row.restDayPayTerm === 'paid' ? ' selected' : '') + '>' + L('Yes — paid rest days', '是 — 有薪休息日') + '</option><option value="unpaid"' + (row.restDayPayTerm === 'unpaid' ? ' selected' : '') + '>' + L('No — unpaid rest days', '否 — 無薪休息日') + '</option></select>' +
        '<label for="rh-work-' + i + '">' + L('If a rest day is worked voluntarily, what was agreed?', '如自願在休息日上班，雙方有甚麼協議？') + '</label><select id="rh-work-' + i + '"><option value="">' + L('Choose…', '請選擇……') + '</option><option value="no_extra_payment"' + (row.restDayWorkArrangement === 'no_extra_payment' ? ' selected' : '') + '>' + L('No extra cash agreed', '沒有協議額外現金') + '</option><option value="agreed_payment"' + (row.restDayWorkArrangement === 'agreed_payment' ? ' selected' : '') + '>' + L('Extra cash amount agreed', '已協議額外現金') + '</option><option value="substituted_rest_day"' + (row.restDayWorkArrangement === 'substituted_rest_day' ? ' selected' : '') + '>' + L('Another rest day agreed', '已協議另一天休息') + '</option></select>' +
