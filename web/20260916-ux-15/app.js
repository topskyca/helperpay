...
(function () {
  ...
  const APP_VERSION = '1.1.5-web-free';
  ...
  function salaryHtml() {
    ...
    if (!stmt.historicalOnly && !(frozen?.inputs?.recordReview?.employerConfirmed && workReview.ready && !changed)) html += '<section class="legal-status mt" id="salary-work-review"><h2>' +
      (workReview.ready && assessment.ready ? L('Review recorded work before confirming pay', '確認薪金前核對工作記錄') :
        workReview.ready ? L('Work records are saved; pay details still need review', '工作記錄已保存；薪金資料仍須核對') :
        L('Work records still need review', '工作記錄仍須核對')) + '</h2>' +
